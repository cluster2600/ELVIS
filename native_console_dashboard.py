...
class NativeConsoleDashboard:
    ...
    def _draw_info_pane(self, start_y: int, start_x: int):
        ...
        self.safe_addstr(y + 2, start_x, "Status: LIVE TRADING", curses.color_pair(1) | curses.A_BOLD)
        ...
        if open_positions:
            for i, pos in enumerate(open_positions[:4]):  # Show up to 4 open positions
                ...
                try:
                    ...
                    pnl_color = curses.color_pair(1) if unrealized_pnl >= 0 else curses.color_pair(2)
        ...
        self.safe_addstr(y, start_x, "--- System Info ---", curses.color_pair(3) | curses.A_BOLD)
        
        y += 1
        # Check API health
        health = self.get_api_data("/health")
        api_status = "HEALTHY" if health and health.get('status') == 'healthy' else "OFFLINE"
        api_color = curses.color_pair(1) if api_status == "HEALTHY" else curses.color_pair(2)
        self.safe_addstr(y, start_x, f"API Status: {api_status}", api_color | curses.A_BOLD)
    ...
    def get_ohlc_data(self):
        ...
        try:
            ...
            response = requests.get("https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1m&limit=40", timeout=3)
            data = response.json()
    ...
    def _draw_chart_pane(self, start_y: int, start_x: int, height: int, width: int):
        ...
        self.safe_addstr(start_y, start_x, "--- BTC/USDT Candlestick Chart (1m) ---", curses.color_pair(3) | curses.A_BOLD)
        ...
        candles = ohlc_data[-chart_width:] if len(ohlc_data) >= chart_width else ohlc_data
        ...
        self.safe_addstr(indicator_y, start_x + 15, f"SMA(20): ${sma_val:.0f}", curses.color_pair(6))
        
        # Current price display
        price_change = ((current_price - candles[0]['close']) / candles[0]['close'] * 100) if len(candles) > 1 else 0
        price_color = curses.color_pair(1) if price_change >= 0 else curses.color_pair(2)
        change_sign = "+" if price_change >= 0 else ""
        
        self.safe_addstr(indicator_y + 1, start_x, f"Price: ${current_price:.2f} ({change_sign}{price_change:.2f}%)", price_color)
    
    def _draw_market_depth_pane(self, start_y: int, start_x: int, height: int, width: int):
        """Draw the right pane with market depth (columns 94-120)"""
        y = start_y
        
        # Right pane marker (as per original)
        max_y, max_x = self.stdscr.getmaxyx()
        self.safe_addstr(y, start_x, f"RIGHT PANE (cols {start_x}-{max_x-2})", curses.color_pair(4))
        
        y += 2
        self.safe_addstr(y, start_x, "--- Market Depth ---", curses.color_pair(3) | curses.A_BOLD)
        ...
        # API Status
        y += len(bids) + 2
        self.safe_addstr(y, start_x, "--- API Status ---", curses.color_pair(3) | curses.A_BOLD)
        ...
        # Position Sizing
        y += 3
        self.safe_addstr(y, start_x, "--- Position Sizing ---", curses.color_pair(3) | curses.A_BOLD)
    ...
    def _draw_console_messages(self, start_y: int, start_x: int, height: int, width: int):
        ...
        self.safe_addstr(y, start_x, "--- Console Messages ---", curses.color_pair(3) | curses.A_BOLD)
    ...
    def _draw_frame(self):
        ...
        try:
            ...
            chart_pane_width = max(35, max_x - left_pane_width - right_pane_width - 3)  # Much wider minimum chart
            ...
            if market_depth_height > 10:
                self._draw_market_depth_pane(9, right_pane_x + 2, market_depth_height, right_pane_width - 2)
    ...
    def run(self):
        ...
        def main(stdscr):
            ...
            # Initialize colors
            curses.start_color()
            curses.init_pair(1, curses.COLOR_GREEN, curses.COLOR_BLACK)   # Green for profits
            curses.init_pair(2, curses.COLOR_RED, curses.COLOR_BLACK)     # Red for losses
            curses.init_pair(3, curses.COLOR_YELLOW, curses.COLOR_BLACK)  # Yellow for headers
            ...
            curses.init_pair(6, curses.COLOR_WHITE, curses.COLOR_BLACK)   # White for normal text
