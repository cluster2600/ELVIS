...
def train_on_all_trades(df, epochs=20, batch_size=50, debug=False, args=None):
    ...
    if len(df) > 1000:
        logger.info(f"📊 Large dataset detected ({len(df)} trades). Sampling 1000 for LLM analysis...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train LLM-enhanced models on ALL individual paper trades')
    parser.add_argument('--method', type=str, default='auto', help='Training method (auto, all_trades)')
    ...
    parser.add_argument('--batch-size', type=int, default=10, help='LLM processing batch size (smaller for slow LLMs)')
    ...
    try:
        ...
        print(f"🧠 Your local LLM has been trained on ALL {metadata['training_config']['total_trades_loaded']:,} paper trades!")
