...
def load_all_individual_trades(include_test_trades=False, max_trades=0):
    ...
    logger.info("🐘 Loading ALL individual paper trades from PostgreSQL (Patient Mode)...")
...
def create_trade_level_features(df):
    ...
    for symbol in df['symbol'].unique():
        ...
        df.loc[symbol_mask, 'price_change_abs'] = df.loc[symbol_mask, 'price_change'].abs()
        
        # Moving averages (last N trades)
        df.loc[symbol_mask, 'price_sma_5'] = symbol_df['price'].rolling(5, min_periods=1).mean()
        df.loc[symbol_mask, 'price_sma_10'] = symbol_df['price'].rolling(10, min_periods=1).mean()
        df.loc[symbol_mask, 'price_sma_20'] = symbol_df['price'].rolling(20, min_periods=1).mean()
        
        # Price volatility (rolling standard deviation)
        df.loc[symbol_mask, 'price_volatility_5'] = symbol_df['price'].rolling(5, min_periods=1).std()
        df.loc[symbol_mask, 'price_volatility_10'] = symbol_df['price'].rolling(10, min_periods=1).std()
        
        # Position relative to moving averages
        df.loc[symbol_mask, 'price_vs_sma5'] = (df.loc[symbol_mask, 'price'] / df.loc[symbol_mask, 'price_sma_5']) - 1
        df.loc[symbol_mask, 'price_vs_sma10'] = (df.loc[symbol_mask, 'price'] / df.loc[symbol_mask, 'price_sma_10']) - 1
        
        # RSI-like momentum indicator
        price_changes = df.loc[symbol_mask, 'price_change']
        gains = price_changes.where(price_changes > 0, 0).rolling(14, min_periods=1).mean()
        losses = (-price_changes.where(price_changes < 0, 0)).rolling(14, min_periods=1).mean()
        rs = gains / (losses + 1e-10)
        df.loc[symbol_mask, 'rsi'] = 100 - (100 / (1 + rs))
        
        # Volume features
        df.loc[symbol_mask, 'volume_sma'] = symbol_df['quantity'].rolling(10, min_periods=1).mean()
        df.loc[symbol_mask, 'volume_ratio'] = df.loc[symbol_mask, 'quantity'] / (df.loc[symbol_mask, 'volume_sma'] + 1e-10)
    ...
    df['time_since_last'] = df.groupby('symbol')['timestamp'].diff().dt.total_seconds() / 60  # minutes
    ...
    for symbol in df['symbol'].unique():
        ...
        df.loc[symbol_mask, 'cumulative_volume'] = df.loc[symbol_mask, 'quantity'].cumsum()
        df.loc[symbol_mask, 'running_avg_price'] = df.loc[symbol_mask, 'price'].expanding().mean()
    ...
    logger.info(f"   Features per trade: {len([col for col in df.columns if col not in ['id', 'timestamp', 'symbol', 'side']])}")
...
def create_trade_level_targets(df, horizon=5):
    ...
    for symbol in df['symbol'].unique():
        ...
        df.loc[symbol_mask, f'future_price_{horizon}'] = symbol_data['price'].shift(-horizon)
        
        # Future return N trades ahead
        future_return = (df.loc[symbol_mask, f'future_price_{horizon}'] / df.loc[symbol_mask, 'price']) - 1
        df.loc[symbol_mask, f'future_return_{horizon}'] = future_return
        
        # Classification targets
        df.loc[symbol_mask, 'target_up'] = (future_return > 0).astype(int)
        df.loc[symbol_mask, 'target_profitable'] = (future_return > 0.001).astype(int)  # >0.1% profit
    ...
    logger.info(f"   Target PROFITABLE distribution: {dict(df['target_profitable'].value_counts())}")
    logger.info(f"   Return range: [{df['target_return'].min():.6f}, {df['target_return'].max():.6f}]")
...
def train_patient_llm_models(df, epochs=20, batch_size=5, debug=False, args=None):
    ...
    logger.info(f"🐌 Training Patient LLM-enhanced models on {len(df)} individual trades...")
    ...
    if len(df) > sample_size:
        logger.info(f"📊 Large dataset detected ({len(df)} trades). Using patient sampling ({sample_size} samples)...")
        ...
        logger.info(f"⏰ Using {batch_size} batch size and 5-minute timeout per request...")
        
        start_time = time.time()
        enhanced_sample = trainer.prepare_llm_features(llm_sample_df, batch_size=batch_size)
        end_time = time.time()
        
        llm_processing_time = end_time - start_time
        logger.info(f"⏱️ LLM processing took {llm_processing_time/60:.1f} minutes for {sample_size} samples")
        ...
        logger.info(f"⏰ This may take {len(df) * batch_size * 5 / 60:.1f} minutes with patient processing...")
    ...
    exclude_columns = ['id', 'timestamp', 'symbol', 'side', 'target_up', 'target_profitable', 'target_return']
    exclude_columns.extend([col for col in enhanced_df.columns if col.startswith('future_')])
    exclude_columns.extend([col for col in enhanced_df.columns if col.startswith('target_')])
    ...
    metadata = {
        ...
        'training_config': {
            'method': 'patient_llm',
            'total_trades_requested': 'all_available' if (args and args.trades == 0) else (args.trades if args else 'unknown'),
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train Patient LLM-enhanced models on ALL individual paper trades')
    parser.add_argument('--trades', type=int, default=0, help='Max number of trades (0 = infinite/all available)')
    parser.add_argument('--epochs', type=int, default=20, help='Number of training epochs')
    parser.add_argument('--horizon', type=int, default=5, help='Prediction horizon in trades ahead')
    parser.add_argument('--batch-size', type=int, default=5, help='LLM batch size (very small for patient processing)')
    parser.add_argument('--include-test', action='store_true', help='Include TEST trades in training')
    parser.add_argument('--debug', action='store_true', help='Enable debug logging')
    parser.add_argument('--vault', action='store_true', default=True, help='Enable Vault for secrets management')
    ...
    try:
        ...
        trades_df = load_all_individual_trades(include_test_trades=args.include_test, max_trades=args.trades)
        ...
        logger.info(f"✅ Regression MSE: {metadata['model_performance']['regression_mse']:.8f}")
        ...
        print(f"   Trades Requested: {metadata['training_config']['total_trades_requested']}")
        print(f"   Trades Loaded: {metadata['training_config']['total_trades_loaded']:,}")
        ...
        print(f"   OOB Score (Classification): {metadata['model_performance']['oob_score_classification']:.4f}")
        print(f"   OOB Score (Regression): {metadata['model_performance']['oob_score_regression']:.4f}")
        ...
        print(f"🐌 Your slow LLM was given plenty of time to analyze {metadata['training_config']['llm_sample_size']:,} trades!")
