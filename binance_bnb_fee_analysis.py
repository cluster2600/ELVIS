...
class EnhancedBinanceFeeCalculator:
    ...
    def get_effective_fee_rate(self, base_fee: float, use_bnb: bool, is_futures: bool = True) -> float:
    ...
    def calculate_trading_fee(self, price: float, quantity: float, is_maker: bool = False, 
                            is_futures: bool = True, use_bnb: bool = False) -> dict:
        """
        ...
        if is_futures:
            base_fee_rate = self.futures_maker_fee if is_maker else self.futures_taker_fee
        else:
        ...
        effective_fee_rate = self.get_effective_fee_rate(base_fee_rate, use_bnb, is_futures)
        ...
        return {
            ...
            'discount_percent': (savings / standard_fee_usdt * 100) if standard_fee_usdt > 0 else 0,
            'effective_fee_rate': effective_fee_rate
        }
    
    def analyze_bnb_vs_btc_trading(self, btc_price: float, bnb_price: float, trade_amount_usdt: float) -> dict:
        """
        Compare trading BNB/USDT vs BTC/USDT with fee considerations
        """
        # BTC trade analysis
        btc_quantity = trade_amount_usdt / btc_price
        btc_fee_standard = self.calculate_trading_fee(btc_price, btc_quantity, is_futures=True, use_bnb=False)
        btc_fee_bnb = self.calculate_trading_fee(btc_price, btc_quantity, is_futures=True, use_bnb=True)
        
        # BNB trade analysis  
        bnb_quantity = trade_amount_usdt / bnb_price
        bnb_fee_standard = self.calculate_trading_fee(bnb_price, bnb_quantity, is_futures=True, use_bnb=False)
        bnb_fee_bnb = self.calculate_trading_fee(bnb_price, bnb_quantity, is_futures=True, use_bnb=True)
...
def main():
    ...
    logger.info(f"{'Trade Size':<12} {'Standard Fee':<15} {'With BNB':<15} {'Savings':<12} {'Savings %'}")
    logger.info("-" * 70)
    
    for trade_size in trade_sizes:
        btc_quantity = trade_size / btc_price
        
        fee_standard = calc.calculate_trading_fee(btc_price, btc_quantity, is_futures=True, use_bnb=False)
        fee_bnb = calc.calculate_trading_fee(btc_price, btc_quantity, is_futures=True, use_bnb=True)
        
        logger.info(f"${trade_size:<11,} ${fee_standard['standard_fee_usdt']:<14.6f} "
    ...
    logger.info(f"  Standard fee: ${comparison['btc']['fees_standard']['standard_fee_usdt']:.6f}")
    logger.info(f"  With BNB fee: ${comparison['btc']['fees_with_bnb']['discounted_fee_usdt']:.6f}")
    logger.info(f"  BNB savings: ${comparison['btc']['fees_with_bnb']['savings_usdt']:.6f}")
    
    logger.info("\nBNB/USDT Trade:")
    logger.info(f"  Quantity: {comparison['bnb']['quantity']:.6f} BNB")
    logger.info(f"  Standard fee: ${comparison['bnb']['fees_standard']['standard_fee_usdt']:.6f}")
    logger.info(f"  With BNB fee: ${comparison['bnb']['fees_with_bnb']['discounted_fee_usdt']:.6f}")
    logger.info(f"  BNB savings: ${comparison['bnb']['fees_with_bnb']['savings_usdt']:.6f}")
    ...
    logger.info(f"\nFutures Trading:")
    logger.info(f"  Min BNB needed: {futures_breakeven['min_bnb_purchase']} BNB (${futures_breakeven['min_bnb_cost_usdt']:.2f})")
    logger.info(f"  Discount: {futures_breakeven['discount_percent']:.0f}%")
    logger.info(f"  Breakeven volume: ${futures_breakeven['breakeven_volume_usdt']:,.0f}")
    
    logger.info(f"\nSpot Trading:")
    logger.info(f"  Min BNB needed: {spot_breakeven['min_bnb_purchase']} BNB (${spot_breakeven['min_bnb_cost_usdt']:.2f})")
    ...
    if futures_breakeven['breakeven_volume_usdt'] < 1000:
        ...
        logger.info(f"   You break even with just ${futures_breakeven['breakeven_volume_usdt']:,.0f} in trading volume")
    else:
    ...
    if spot_breakeven['breakeven_volume_usdt'] < 1000:
        ...
        logger.info("⚠️ BNB fee discount for spot: only beneficial for high-volume traders")
