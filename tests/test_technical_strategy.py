...
class TestTechnicalStrategy(unittest.TestCase):
    ...
    def test_calculate_position_size(self):
        ...
        position_size = self.strategy.calculate_position_size(
            self.test_data, current_price, available_capital
        )

        # Check result
        expected_size = (
            available_capital * self.strategy.position_size_pct
        ) / current_price
    ...
    def test_calculate_take_profit(self):
        ...
        expected_take_profit = max(
            entry_price + (3 * atr), entry_price * (1 + self.strategy.take_profit_pct)
        )
