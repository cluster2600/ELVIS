...
from utils.paper_trade_db import init_db, add_open_position, get_open_positions, record_trade
...
def test_position_sizing():
    ...
    position_size = strategy.calculate_position_size(mock_data, current_price, available_capital)
    ...
    logger.info(f"  % of Capital: {(position_size * current_price / available_capital) * 100:.2f}%")
...
def test_position_display():
    ...
    for i, pos in enumerate(open_positions):
        ...
        logger.info(f"  {i+1}. {symbol} {side} {abs(size):.6f} @ ${entry_price:.2f} | PnL: ${pnl:.2f}")
...
def test_executor_positions():
    ...
    strategy = EnsembleStrategy(logger=logger)
    position_size = strategy.calculate_position_size(mock_data, current_price, total_balance)
