                ...
                for symbol, signal_info in signals.items():
                    ...
                    if signal in ['BUY', 'SELL'] and confidence > 0.01:
                        ...
                        position_size = strategy.calculate_position_size(
                            test_data, current_price, available_balance, leverage, confidence
                        )
                        ...
                        if signal == 'BUY':
                            ...
                            if order_result:
                                print(f"✅ BUY ORDER EXECUTED: {position_size:.6f} {symbol} at ${current_price:.2f}")
                            else:
                                print(f"❌ BUY order failed")
                        elif signal == 'SELL':
                            order_result = executor.place_order(symbol, 'sell', position_size, current_price)
                            if order_result:
                                print(f"✅ SELL ORDER EXECUTED: {position_size:.6f} {symbol} at ${current_price:.2f}")
                            else:
                                print(f"❌ SELL order failed")
                    else:
                        print(f"⏸️  No trade: signal={signal}, confidence={confidence:.3f} (threshold=0.01)")
        ...
        try:
            ...
            if recent_trades:
                ...
                for trade in recent_trades:
                    if len(trade) >= 7:
                        print(f"   - {trade[1]} | {trade[2]} | {trade[3]} | ${trade[4]:.2f} | {trade[5]:.6f} | PnL: ${trade[6]:.2f}")
            else:
