...
def test_right_pane():
    ...
    print(
        f"Right pane spans columns: {right_pane_x} to {right_pane_x + right_pane_width}"
    )
    print()
    ...
    def capture_right_pane(y, x, text, *args):
        ...
        if x >= right_pane_x and x < right_pane_x + right_pane_width:
            right_pane_content.append(
                {"y": y, "x": x, "text": text, "relative_x": x - right_pane_x}
            )
    ...
    try:
        dashboard._draw_volume_profile_pane(
            market_depth_y, market_depth_x, market_depth_height, market_depth_width
        )
        market_depth_items = [
            item
            for item in right_pane_content
            if "Market Depth" in item["text"]
            or "ASKS" in item["text"]
            or "BIDS" in item["text"]
        ]
        ...
        if market_depth_items:
            ...
            for item in market_depth_items[:5]:
                print(
                    f"      Row {item['y']:2d}, Col {item['relative_x']:2d}: {item['text']}"
                )
        else:
    ...
    try:
        ...
        dashboard._draw_position_sizing_pane(
            position_sizing_y,
            position_sizing_x,
            position_sizing_height,
            position_sizing_width,
        )
    ...
    if len(right_pane_content) > 0:
        ...
        print(
            f"   Content spans rows {min(item['y'] for item in right_pane_content)} to {max(item['y'] for item in right_pane_content)}"
        )
        print(
            f"   Content spans columns {right_pane_x + min(item['relative_x'] for item in right_pane_content)} to {right_pane_x + max(item['relative_x'] for item in right_pane_content)}"
        )

        print("\n🎯 IN THE DASHBOARD, LOOK FOR:")
        print(
            f"   - Right side of terminal (columns {right_pane_x}-{right_pane_x + right_pane_width})"
        )
        ...
        market_samples = [
            item
            for item in right_pane_content
            if "Market" in item["text"]
            or "ASKS" in item["text"]
            or "Price" in item["text"]
        ][:3]
