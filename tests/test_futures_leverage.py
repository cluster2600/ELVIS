...
def test_futures_executor():
    ...
    executor = BinanceExecutor(
        logger=logger, is_testnet=True, use_futures=True, default_leverage=10
    )

    success = executor.initialize()
    logger.info(
        f"Futures executor initialization: {'SUCCESS' if success else 'FAILED'}"
    )
...
def test_leverage_position_sizing():
    ...
    for leverage in leverage_levels:
        ...
        logger.info(
            f"  % of Capital: {(actual_capital_used / available_capital) * 100:.2f}%"
        )
        logger.info("")
...
def test_leverage_trades():
    ...
    sell_result = executor.execute_sell(
        "BTCUSDT", position_size, current_price + 100
    )  # Small profit
...
def test_configuration():
    ...
    logger.info(
        f"Leverage Range: {TRADING_CONFIG['LEVERAGE_MIN']}x - {TRADING_CONFIG['LEVERAGE_MAX']}x"
    )
    ...
    logger.info(
        f"  Risk per trade: {risk_per_trade * 100}% = ${capital * risk_per_trade}"
    )
    logger.info(
        f"  With {leverage}x leverage: ${capital * risk_per_trade * leverage} exposure"
    )
    logger.info(
        f"  Actual capital at risk: ${capital * risk_per_trade} (same as without leverage)"
    )
