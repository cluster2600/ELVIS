...
def test_pytorch_warnings():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        try:
            ...
            tensor_warnings = [warning for warning in w 
                             if 'Creating a tensor from a list of numpy.ndarrays is extremely slow' in str(warning.message)]
...
def main():
    ...
    logger.info(f"   PyTorch Tensor Creation Warnings: {'✅' if test2_success else '❌'}")
