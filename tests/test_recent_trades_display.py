...
def test_recent_trades_display():
    ...
    if profitable_trades:
        for i, trade in enumerate(profitable_trades[:5]):  # Show top 5
            ...
            logger.info(f"{i+1}. {timestamp} | {side} {quantity:.4f} {symbol} @ ${price:.2f} | PnL: {pnl_symbol}${pnl:.2f}")
    else:
    ...
    for i, trade in enumerate(recent_activity[:5]):
        ...
        logger.info(f"{i+1}. {timestamp} | {side} {quantity:.4f} {symbol} @ ${price:.2f} | Val: ${trade_value:.0f}")
