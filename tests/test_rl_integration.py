...
def test_rl_model_standalone():
    ...
    try:
        ...
        logger.info(f"RL Model standalone prediction: {signal} (confidence: {confidence:.3f})")
        
        # Test with different market conditions
        test_cases = [
            {'price': 119000.0, 'volume': 1000.0, 'rsi': 25.0, 'macd': 0.5},  # Oversold
            {'price': 119000.0, 'volume': 1000.0, 'rsi': 75.0, 'macd': -0.3}, # Overbought
            {'price': 119000.0, 'volume': 1000.0, 'rsi': 50.0, 'macd': 0.0},  # Neutral
        ]
        
        for i, test_case in enumerate(test_cases):
            signal, confidence = rl_model.predict_action(test_case)
            logger.info(f"Test case {i+1}: {signal} ({confidence:.3f}) - RSI: {test_case['rsi']}, MACD: {test_case['macd']}")
...
def test_ensemble_integration():
    ...
    try:
        ...
        signal, confidence = ensemble_strategy.generate_signal('BTCUSDT', test_market_data)
        logger.info(f"Ensemble Strategy prediction: {signal} (confidence: {confidence:.3f})")
        
        # Test with different market conditions
        test_cases = [
            {'price': 119000.0, 'close': 119000.0, 'volume': 1000.0, 'rsi': 25.0, 'macd': 0.5, 'macd_signal': 0.3},  # Oversold
            {'price': 119000.0, 'close': 119000.0, 'volume': 1000.0, 'rsi': 75.0, 'macd': -0.3, 'macd_signal': -0.1}, # Overbought
            {'price': 119000.0, 'close': 119000.0, 'volume': 1000.0, 'rsi': 50.0, 'macd': 0.0, 'macd_signal': 0.0},  # Neutral
        ]
        
        for i, test_case in enumerate(test_cases):
            # Fill in missing fields with defaults
            complete_test_case = {**test_market_data, **test_case}
            signal, confidence = ensemble_strategy.generate_signal('BTCUSDT', complete_test_case)
            logger.info(f"Ensemble test case {i+1}: {signal} ({confidence:.3f}) - RSI: {test_case['rsi']}, MACD: {test_case['macd']}")
...
def test_trade_result_update():
    ...
    try:
        ...
        trade_results = [
            {'price': 119000.0, 'pnl': 1.50, 'fees': 0.48, 'side': 'BUY', 'quantity': 0.001},
            {'price': 119200.0, 'pnl': -0.80, 'fees': 0.48, 'side': 'SELL', 'quantity': 0.001},
            {'price': 119100.0, 'pnl': 2.10, 'fees': 0.48, 'side': 'BUY', 'quantity': 0.001},
        ]
        ...
        for i, trade_result in enumerate(trade_results):
            ...
            logger.info(f"Updated RL model with trade result {i+1}: PnL=${trade_result['pnl']:.2f}")
