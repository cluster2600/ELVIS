...
class TestEnsembleModel:
    ...
    def test_ensemble_model_initialization(self, mock_logger):
        ...
        models = [("rf", rf_model), ("nn", nn_model)]

        ensemble = EnsembleModel(logger=mock_logger, models=models, weights=[0.5, 0.5])
    ...
    def test_save_and_load_ensemble_config(self, mock_logger, tmp_path):
        ...
        # Load configuration
        with patch(
            "core.models.random_forest_model.RandomForestModel.load"
        ) as mock_rf_load:
            with patch(
                "core.models.neural_network_model.NeuralNetworkModel.load"
            ) as mock_nn_load:
