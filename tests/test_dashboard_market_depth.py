    ...
    if dashboard.price_fetcher:
        ...
        if order_book:
            ...
            if bids_data and asks_data:
                logger.info(
                    f"✅ Order book contains {len(bids_data)} bids and {len(asks_data)} asks"
                )
...
if __name__ == "__main__":
    ...
    if success:
        ...
        logger.info(
            "The console dashboard market depth should now show live order book data."
        )
        logger.info(
            "Run 'python utils/console_dashboard.py' to see the updated dashboard."
        )
    else:
