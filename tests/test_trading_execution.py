...
def test_trading_execution():
    ...
    logger.info(f"Mock data created: RSI={data['rsi'].iloc[-1]}, MACD={data['macd'].iloc[-1]}, Price={data['close'].iloc[-1]:.2f}")
    ...
    if signal in ['BUY', 'SELL'] and confidence > 0.1:
        ...
        position_size = min(0.001, available_balance * 0.01 / current_price)  # 1% of balance
