...
class TestIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Set up test data
        self.start_date = (datetime.now() - timedelta(days=90)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    def test_data_processing_to_strategy(self):
        ...
        tf_buy, tf_sell = trend_following_strategy.generate_signals(
    ...
    def test_end_to_end_workflow(self):
        ...
        risk_manager = RiskManager(
            executor=MagicMock(spec=BinanceExecutor), logger=self.logger
        )
        performance_monitor = PerformanceMonitor()
