...
def test_working_llm():
    ...
    print(
        f"📊 Sentiment: {sentiment['sentiment']} ({sentiment['confidence']:.1%} confidence)"
    )
    ...
    print(
        f"📈 Confidence: {enhancement.get('original_confidence', 0.85):.1%} → {enhancement['adjusted_confidence']:.1%}"
    )
