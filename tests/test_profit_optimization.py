...
def test_profit_optimizations():
    ...
    try:
        ...
        ), f"Cooldown should be 0, got {rm.cooldown_period}"
        assert (
            rm.leverage_target >= 100.0
        ), f"Leverage should be 100x, got {rm.leverage_target}"
        assert (
            rm.max_trades_per_day >= 1000
        ), f"Max trades too low: {rm.max_trades_per_day}"
    ...
    try:
        ...
        assert hasattr(
            strategy, "profit_optimization_mode"
        ), "Missing profit optimization mode"
        assert (
            strategy.profit_optimization_mode == True
        ), "Profit optimization not enabled"
    ...
    try:
        from trading.strategies.high_leverage_scalping_strategy import (
            HighLeverageScalpingStrategy,
        )
        ...
        print(
            f"✅ Profit target: {scalping_strategy.scalp_profit_target}% = {scalping_strategy.scalp_profit_target * scalping_strategy.leverage}% with leverage"
        )
        print(
            f"✅ Stop loss: {scalping_strategy.stop_loss_pct}% = {scalping_strategy.stop_loss_pct * scalping_strategy.leverage}% with leverage"
        )
        print(f"✅ Min confidence: {scalping_strategy.min_confidence:.1%}")

        # Test signal generation
        signal, confidence = scalping_strategy.generate_signal(
            "BTCUSDT", mock_market_data
        )
