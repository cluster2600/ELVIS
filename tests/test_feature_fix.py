...
def test_research_strategy_features():
    ...
    try:
        ...
        if features.shape[1] == 9:
            ...
            logger.error(
                f"   ❌ Wrong feature count: got {features.shape[1]}, expected 9"
            )
            return False
        ...
        if "BTCUSDT" in signals:
            ...
            logger.info(
                f"   Generated signal: {signal['signal']} (confidence: {signal['confidence']:.3f})"
            )
...
def test_model_loading():
    ...
    try:
        ...
        if model.n_features_in_ == 9:
            ...
            logger.error(
                f"   ❌ Model expects {model.n_features_in_} features, should be 9"
            )
            return False
