...
def test_forced_trading():
    ...
    try:
        ...
        position_size = strategy.calculate_position_size(
            mock_data, current_price, available_balance, leverage=10, signal_confidence=0.9
        )
        ...
        buy_result = executor.place_order("BTCUSDT", "buy", position_size, current_price)
        
        if buy_result:
            print(f"✅ BUY order successful: {position_size:.6f} BTC at ${current_price}")
        ...
        sell_result = executor.place_order("BTCUSDT", "sell", position_size, current_price)
        
        if sell_result:
            print(f"✅ SELL order successful: {position_size:.6f} BTC at ${current_price}")
            time.sleep(1)  # Wait for execution
        else:
        ...
        try:
            ...
            if trade_count > 0:
                ...
                for trade in recent_trades:
                    ...
                    if len(trade) >= 7:
                        print(f"  {trade[2]} {trade[3]} {trade[5]:.6f} @ ${trade[4]:.2f} | PnL: ${trade[6]:.2f}")
            else:
