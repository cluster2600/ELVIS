...
def test_api_endpoints():
    ...
    try:
        ...
        with app.test_client() as client:
            ...
            print(f"✓ /api/exchanges/prices endpoint exists (status: {response.status_code})")
            
            response = client.get('/api/arbitrage/opportunities')
            print(f"✓ /api/arbitrage/opportunities endpoint exists (status: {response.status_code})")
            
            response = client.get('/api/portfolio/consolidated')
            print(f"✓ /api/portfolio/consolidated endpoint exists (status: {response.status_code})")
            
            response = client.get('/api/exchanges/health')
            print(f"✓ /api/exchanges/health endpoint exists (status: {response.status_code})")
...
def test_ensemble_strategy_enhancements():
    ...
    try:
        ...
        assert hasattr(strategy, 'check_arbitrage_opportunities'), "Missing arbitrage method"
        print("✓ check_arbitrage_opportunities method exists")
        
        assert hasattr(strategy, 'execute_multi_exchange_order'), "Missing multi-exchange order method"
        print("✓ execute_multi_exchange_order method exists")
        
        assert hasattr(strategy, 'get_consolidated_portfolio'), "Missing consolidated portfolio method"
        print("✓ get_consolidated_portfolio method exists")
        
        assert hasattr(strategy, 'get_market_overview'), "Missing market overview method"
