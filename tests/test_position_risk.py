...
def test_position_risk_calculations():
    ...
    if recent_trades:
        ...
        for i, trade in enumerate(recent_trades[:5]):
            if len(trade) >= 7:
                ...
                logger.info(f"Trade {i+1}: {trade[3]} | Volume: ${volume:,.2f} | PnL: ${pnl:.2f}")
        ...
        if recent_volumes:
            ...
            logger.info(f"Average Trade Size: ${avg_trade_size:,.2f} ({avg_risk_pct:.1f}% of portfolio)")
            logger.info(f"Maximum Trade Size: ${max_trade_size:,.2f} ({max_risk_pct:.1f}% of portfolio)")
            ...
            if len(recent_pnls) > 1:
                ...
                # Risk assessment
                if avg_risk_pct > 10:
                    logger.warning("⚠️  HIGH RISK: Average trade size > 10% of portfolio")
                elif avg_risk_pct > 5:
                    logger.info("⚡ MODERATE RISK: Average trade size 5-10% of portfolio")
                else:
    ...
    for risk_pct in risk_levels:
        ...
        logger.info(f"  {risk_pct*100:.0f}% Risk: {position_size:.4f} BTC (${position_value:,.0f} exposure)")
