...
def test_live_pnl_calculations():
    ...
    logger.info(f"Neutral trades: {len(all_trades) - profitable_trades - losing_trades}")
    ...
    if open_positions:
        ...
        for i, pos in enumerate(open_positions):
            if len(pos) >= 4:
                ...
                logger.info(f"Position {i+1}: {symbol} | Entry: ${entry_price:.2f} | "
...
def test_real_time_updates():
    ...
    for i, trade in enumerate(recent_trades):
        if len(trade) >= 7:
            ...
            logger.info(f"{i+1}. {timestamp} | {trade[3]} | PnL: ${pnl:.2f} | Running Total: ${running_pnl:.2f}")
