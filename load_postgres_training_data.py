...
def postgres_trades_to_ohlcv(df, timeframe='5min'):
    ...
    try:
        ...
        ohlcv.columns = ['open', 'high', 'low', 'close', 'volume', 'pnl_total', 'fees_total', 'trade_count']
        ...
        logger.info(f"   Price range: ${ohlcv['low'].min():,.2f} - ${ohlcv['high'].max():,.2f}")
...
def enhance_postgres_training_data(df):
    ...
    logger.info(f"   Target distribution: UP={df['target_up'].sum()}, DOWN={len(df) - df['target_up'].sum()}")
    logger.info(f"   Return range: {df['target_return'].min():.4f} to {df['target_return'].max():.4f}")
