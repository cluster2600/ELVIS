...
def main():
    ...
    parser.add_argument('--trades', type=int, default=500, help='Number of trades to train on')
    parser.add_argument('--model-path', type=str, default='models/trading_rl_model.pth', help='Path to save model')
    parser.add_argument('--retrain', action='store_true', help='Force retrain even if model exists')
    parser.add_argument('--analyze', action='store_true', help='Analyze recent trades first')
...
def test_model_predictions(rl_model):
    ...
    try:
        ...
        test_cases = [
            {'price': current_price * 0.99, 'volume': 1000.0, 'rsi': 30.0, 'macd': 0.5},  # Oversold
            {'price': current_price * 1.01, 'volume': 1200.0, 'rsi': 70.0, 'macd': -0.3}, # Overbought
            {'price': current_price, 'volume': 900.0, 'rsi': 50.0, 'macd': 0.1},   # Neutral
            {'price': current_price * 1.02, 'volume': 1500.0, 'rsi': 45.0, 'macd': 0.8},  # Bullish
            {'price': current_price * 0.98, 'volume': 800.0, 'rsi': 35.0, 'macd': -0.5}   # Bearish
        ]
        
        for i, test_case in enumerate(test_cases):
            signal, confidence = rl_model.predict_action(test_case, recent_trades)
            logger.info(f"Test case {i+1}: {signal} (confidence: {confidence:.3f}) - {test_case}")
