...
class TradeHistoryProcessor:
    ...
    def load_trade_history(self, limit: Optional[int] = None, exclude_test: bool = True) -> pd.DataFrame:
        """
        ...
        try:
            ...
            self.logger.info(f"Loading trade history: {total_trades} total trades available")
            
            # Load trades from database
            trades = get_all_trades(limit=limit or total_trades, exclude_test=exclude_test)
            ...
            columns = ['id', 'timestamp', 'symbol', 'side', 'price', 'quantity', 'pnl', 'fee']
    ...
    def extract_market_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['price_momentum_5'] = (df['price'] - df['price'].shift(5)) / df['price'].shift(5)
            df['price_momentum_20'] = (df['price'] - df['price'].shift(20)) / df['price'].shift(20)
            
            # High/Low price levels
            df['price_high_10'] = df['price'].rolling(window=10).max()
            df['price_low_10'] = df['price'].rolling(window=10).min()
            df['price_position'] = (df['price'] - df['price_low_10']) / (df['price_high_10'] - df['price_low_10'])
    ...
    def extract_trading_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['position_size_zscore'] = (df['notional_value'] - df['notional_value'].rolling(window=50, min_periods=1).mean()) / df['notional_value'].rolling(window=50, min_periods=1).std().replace(0, np.nan)
    ...
    def create_target_labels(self, df: pd.DataFrame, prediction_horizon: int = 5) -> pd.DataFrame:
        """
        ...
        try:
            ...
            df['future_price_up'] = (df['price'].shift(-prediction_horizon) > df['price']).astype(int)
            
            # Future market bias (based on side distribution)
            df['future_long_bias'] = df['is_long'].rolling(window=prediction_horizon).mean().shift(-prediction_horizon)
            
            # Future volatility
            df['future_volatility'] = df['price'].rolling(window=prediction_horizon, min_periods=1).std().shift(-prediction_horizon)
    ...
    def process_for_training(self, limit: Optional[int] = None, prediction_horizon: int = 5) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        try:
            ...
            feature_columns = [col for col in df.columns if not col.startswith('future_') and col not in ['id', 'timestamp', 'symbol']]
            ...
            self.logger.info(f"Processed {len(features_df)} samples with {len(feature_columns)} features")
    ...
    def get_feature_importance_data(self) -> Dict:
        ...
        if self.processed_features is None:
            self.logger.warning("No processed features available. Run process_for_training first.")
            return {}
        
        try:
            # Only analyze numeric columns for correlations and stats
            numeric_features = self.processed_features.select_dtypes(include=[np.number])
            ...
            feature_stats = {
                ...
                'feature_correlations': numeric_features.corr().to_dict() if len(numeric_features.columns) > 0 else {},
                'feature_means': numeric_features.mean().to_dict() if len(numeric_features.columns) > 0 else {},
                'feature_stds': numeric_features.std().to_dict() if len(numeric_features.columns) > 0 else {}
            }
    ...
    def save_processed_data(self, features_df: pd.DataFrame, targets_df: pd.DataFrame, output_dir: str):
        """
