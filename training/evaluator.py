...
def evaluate_multi_agent(agents, env, episodes=10):
    ...
    """
    total_rewards = {
        f"agent_{i}": [] for i in range(len(agents))
    }  # List to track per-episode rewards
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            next_state, rewards, done, _ = env.step(
                actions
            )  # Assume env handles multi-agent steps
            state = next_state

            for i, reward in enumerate(rewards):
                episode_reward[i] += reward
                total_rewards[f"agent_{i}"].append(
                    reward
                )  # Track per-step rewards for detailed metrics

        collective_rewards.append(
            sum(episode_reward)
        )  # Track collective reward per episode

    average_individual_rewards = {
        key: np.mean(total_rewards[key]) for key in total_rewards
    }
