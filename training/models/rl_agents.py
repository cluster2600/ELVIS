...
class TradingEnvironment:
    ...
    def __init__(self, 
        ...
        import numpy as np
        import numpy as np
        if not (isinstance(data, (np.ndarray, list)) and (len(data) > 0)):
            raise ValueError("Data must be a non-empty numpy array or list")
...
class MultiAgentTradingSystem:
    ...
    def __init__(self, 
                 ...
                 initial_balance: float = 10000.0):
        actual_n_agents = int(n_agents) if isinstance(n_agents, (int, float)) else 3  # Added check for n_agents type
        self.n_agents = actual_n_agents
        self.data = data
        self.initial_balance = initial_balance
        self.agents = []
        self.environments = []
