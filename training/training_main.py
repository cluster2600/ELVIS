    ...
    def train(self, resume_checkpoint: Optional[str] = None):
        ...
        try:
            ...
            for epoch in range(start_epoch, self.config["transformer"]["epochs"]):
                self.logger.info(
                    f"Starting epoch {epoch+1}/{self.config['transformer']['epochs']}"
                )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="Output directory for models and logs"
    )
    parser.add_argument(
    ...
    try:
        ...
        # Generate explanations
        trainer.generate_explanations(trainer.model_trainer.model, rl_agents)
