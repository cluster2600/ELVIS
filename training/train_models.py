...
from training.data.trade_history_processor import (
    TradeHistoryProcessor,  # Import trade history processor
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Include trade history from database",
    )
...
class TrainingPipeline:
    ...
    def _setup_distributed(self):
        if self.args.distributed:
            ...
            self.logger.info(
                f"Distributed training enabled. Local rank: {self.args.local_rank}"
            )
    ...
    def _load_and_prepare_data(self):
        ...
        self.data = (
            pd.read_csv(self.args.data)
            if self.args.data.endswith(".csv")
            else pd.read_parquet(self.args.data)
        )
        ...
        if getattr(self.args, "include_trade_history", False):
            ...
            try:
                ...
                trade_features, trade_targets = trade_processor.process_for_training(
                    limit=5000
                )

                if not trade_features.empty:
                    self.logger.info(
                        f"✅ Loaded {len(trade_features)} trade history samples"
                    )
                    self.logger.info(
                        f"📈 Trade features: {len(trade_features.columns)} columns"
                    )
                    self.logger.info(
                        f"🎯 Trade targets: {len(trade_targets.columns)} columns"
                    )

                    # Save trade data for reference
                    trade_processor.save_processed_data(
                        trade_features, trade_targets, "data/processed/trade_history"
                    )
                    ...
                    self.logger.info(
                        "✅ Trade history integration completed successfully"
                    )
                else:
        ...
        if self.X.size == 0 or self.y.size == 0:
            raise ValueError(
                "Feature matrix X or target vector y is empty after data preparation."
            )
        self.logger.info(
            f"Feature matrix X shape: {self.X.shape}, Target vector y shape: {self.y.shape}"
        )

        # Log trade history integration status
        if not self.trade_features.empty:
            self.logger.info(
                f"🔗 Trade history features available: {self.trade_features.shape}"
            )
            self.logger.info(
                f"🔗 Trade history targets available: {self.trade_targets.shape}"
            )
        else:
    ...
    def train_rl_agents(self):
        ...
        rl_agents = MultiAgentTradingSystem(
            rl_config.get("env", {}), rl_config.get("agent", {}).get("n_agents", 1)
        )
    ...
    def evaluate_models(self, rl_agents):
        ...
        args_namespace = SimpleNamespace(
            ...
            eval_times=10,
            target_return=200,
            cwd=self.args.output,
        )
        evaluator = Evaluator(
            cwd=self.args.output, agent_id=0, eval_env=None, args=args_namespace
        )  # Using placeholder for agent_id and eval_env

        try:
            ensemble_models = self.model_trainer.load_ensemble()
            self.logger.info(
                f"Loaded {len(ensemble_models)} ensemble models for evaluation"
            )
            ...
            transformer_metrics = {
                "error": "Failed to load ensemble models",
                "details": str(e),
            }
        ...
        try:
            rl_metrics = (
                rl_agents.evaluate(self.X, self.y)
                if hasattr(rl_agents, "evaluate")
                else {"status": "no_evaluate_method"}
            )
        except Exception as e:
    ...
    def generate_explanations(self, rl_agents):
        ...
        transformer_explanations = self.model_trainer.explain_model(
            self.model_trainer.model, self.X, feature_names
        )
