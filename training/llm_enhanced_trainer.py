...
from core.ai.llm_market_analyzer import LLMMarketAnalyzer, LLMConfig
...
class LLMEnhancedTrainer:
    ...
    def prepare_llm_features(self, market_data: pd.DataFrame, 
        ...
        self.logger.info(f"🧠 Generating LLM features for {len(market_data)} samples...")
        ...
        for i in range(0, len(market_data), batch_size):
            ...
            self.logger.info(f"🔄 Processing LLM batch {i//batch_size + 1}/{total_batches}")
            ...
            for idx, row in batch_data.iterrows():
                try:
                    ...
                    if self.cache_llm_responses and cache_key in self.llm_cache:
                        ...
                        self.logger.debug(f"📋 Using cached LLM features for sample {idx}")
                    else:
    ...
    async def _get_llm_features_for_sample(self, row: pd.Series) -> Dict[str, float]:
        ...
        market_data = {
            ...
            "volatility": float(row.get('volatility', abs(row.get('high', 0) - row.get('low', 0)) / row.get('close', 1) * 100))
        }
    ...
    def _extract_numerical_features(self, analysis) -> Dict[str, float]:
        ...
        bullish_prob = max(0, analysis.price_prediction.get('direction', 0)) * analysis.price_prediction.get('magnitude', 0.5)
        ...
        volatility_prediction = 0.7 if analysis.risk_assessment.get('volatility_warning', False) else 0.4
            ...
            if response.status_code == 200:
                ...
                if analysis_text.strip():
                    ...
                    if non_default_count >= 2:  # At least 2 non-default features
                        ...
                        self.logger.debug(f"⚠️ LLM returned mostly defaults, using intelligent fallback")
                else:
                    self.logger.warning("⚠️ LLM returned empty response")
                    
            else:
                self.logger.warning(f"⚠️ LLM API error {response.status_code}: {response.text}")
    ...
    def _create_analysis_prompt(self, market_data: Dict, technical_analysis: Dict) -> str:
...
sentiment:0.X confidence:0.X bullish:0.X risk:0.X volatility:0.X trend:0.X"""
    ...
    def _extract_numerical_features_from_text(self, analysis_text: str) -> Dict[str, float]:
    ...
    def _get_fallback_features(self, market_data: Dict, technical_analysis: Dict) -> Dict[str, float]:
        ...
        if rsi > 70: sentiment_factors.append(0.8)  # Overbought but bullish
        elif rsi < 30: sentiment_factors.append(0.2)  # Oversold, bearish
        ...
        signal_consistency = 0.8 if abs(rsi - 50) > 15 else 0.5  # High RSI deviation = more confident
        ...
        bullish_prob = bullish_indicators / max(1, total_indicators) if total_indicators > 0 else 0.5
        
        # Risk assessment
        risk_factors = [volatility / 30, abs(price_change) / 15]
        if rsi > 80 or rsi < 20: risk_factors.append(0.8)  # Extreme RSI = risky
        ...
        trend_strength = min(1.0, (abs(price_change) / 5 + abs(macd) * 10 + abs(rsi - 50) / 25) / 3)
    ...
    def save_llm_cache(self, filepath: str):
        ...
        if self.llm_cache:
            ...
            self.logger.info(f"💾 Saved {len(self.llm_cache)} LLM responses to cache: {filepath}")
    ...
    def load_llm_cache(self, filepath: str):
        ...
        if os.path.exists(filepath):
            try:
                ...
                self.logger.info(f"📂 Loaded {len(self.llm_cache)} cached LLM responses from: {filepath}")
            except Exception as e:
