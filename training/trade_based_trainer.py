...
class TradeBasedTrainer:
    ...
    def extract_market_features_at_timestamp(
        self, timestamp: datetime, symbol: str = "BTCUSDT"
    ) -> Dict[str, float]:
        """
    ...
    def extract_trade_outcome_features(self, trade_data: Dict) -> Dict[str, float]:
        ...
        try:
            ...
            features = {
                ...
                "trade_value_usd": trade_data.get("quantity", 0.0)
    ...
    def prepare_training_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for trade in all_trades:
            if len(trade) >= 8 and trade[3] == "SELL":
                ...
                if abs(net_pnl) > 0.50:  # At least 50 cents outcome
                    ...
                    market_features = self.extract_market_features_at_timestamp(
                        trade[1], trade[2]
                    )
        ...
        if len(training_data) < 10:
            self.logger.warning(
                "Insufficient training data. Need more trading history."
            )
    ...
    def create_improved_model(
        ...
        for name, model in models.items():
            try:
                ...
                self.logger.info(
                    f"  CV score: {cv_scores.mean():.3f} (+/- {cv_scores.std() * 2:.3f})"
                )
    ...
    def save_improved_model(
        self,
        model_data: Dict,
        model_path: str = "training/models/trade_learned_model.pkl",
    ):
        """
...
"""
        ...
        if hasattr(model_data["model"], "feature_importances_"):
            ...
            top_features = sorted(
                zip(model_data["feature_names"], importance),
                key=lambda x: x[1],
                reverse=True,
            )
...
def main():
    ...
    if len(X) == 0:
        print(
            "❌ Insufficient training data. Run the bot longer to collect more trades."
        )
        return
