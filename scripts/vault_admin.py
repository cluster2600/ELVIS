...
def update_secret():
    ...
    try:
        ...
        if current_value:
            masked_value = current_value[:4] + "*" * (len(current_value) - 4) if len(current_value) > 4 else "****"
...
def delete_secret():
    ...
    try:
        ...
        # Confirm deletion
        confirm = input(f"Are you sure you want to delete '{name}' from '{category}'? (y/N): ")
        if confirm.lower() != 'y':
...
def test_credentials():
    ...
    try:
        ...
        binance_ok = bool(binance_creds.get('api_key') and binance_creds.get('api_secret'))
...
def main():
    parser = argparse.ArgumentParser(description="Vault Administration for ELVIS Trading Bot")
    ...
    parser.add_argument('--update', action='store_true', help='Update an existing secret')
