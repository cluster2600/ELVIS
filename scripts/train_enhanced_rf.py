...
class EnhancedRFTrainer:
    ...
    def _prepare_database_data(self, historical_days: int, min_samples: int) -> tuple:
        ...
        try:
            ...
            trades = get_all_trades(
                limit=min_samples * 2
            )  # Get more trades to ensure enough data

            if len(trades) < min_samples:
                self.logger.warning(
                    f"Insufficient trades in database: {len(trades)} < {min_samples}"
                )
                return self._generate_synthetic_data(min_samples)
            ...
            if len(training_data) < min_samples:
                self.logger.warning(
                    f"Insufficient processed data: {len(training_data)} < {min_samples}"
                )
                return self._generate_synthetic_data(min_samples)
            ...
            if len(features_df) != len(labels_series):
                ...
                labels_series = labels_series.iloc[-len(features_df) :].reset_index(
                    drop=True
                )

            self.logger.info(
                f"✅ Prepared {len(features_df)} samples with {len(features_df.columns)} features"
            )
    ...
    def _generate_synthetic_data(self, n_samples: int) -> tuple:
        ...
        self.logger.info(
            f"✅ Generated {len(features_df)} synthetic samples with {len(features_df.columns)} features"
        )
    ...
    def train_model(
            ...
            try:
                ...
                if "best_params" in optimization_results:
                    self.model.model = self.model.model.__class__(
                        **optimization_results["best_params"], random_state=42
                    )
        ...
        cv_results = self.model.cross_validate(
            features_df, labels_series, cv_folds=self.config["cross_validation_folds"]
        )
        ...
        training_results["feature_importance"] = feature_importance.head(20).to_dict(
            "records"
        )
        ...
        self.logger.info(
            f"📊 Validation Accuracy: {validation_metrics.get('accuracy', 0):.3f}"
        )
        self.logger.info(f"📊 Validation F1: {validation_metrics.get('f1', 0):.3f}")
        self.logger.info(
            f"📊 Cross-validation Accuracy: {np.mean(cv_results.get('test_accuracy', [0])):.3f}"
        )
    ...
    def test_integration(self) -> dict:
        ...
        self.integration = EnhancedRFIntegration(
            logger=self.logger, model_path=self.config["model_path"]
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train Enhanced Random Forest for ELVIS"
    )
    parser.add_argument(
        "--data-source",
        choices=["database", "csv", "synthetic"],
        default="database",
        help="Data source for training",
    )
    parser.add_argument(
        "--samples", type=int, default=2000, help="Minimum number of training samples"
    )
    parser.add_argument(
    ...
    try:
        ...
        features_df, labels_series = trainer.prepare_training_data(
            data_source=args.data_source, min_samples=args.samples
        )
        ...
        training_results = trainer.train_model(
            features_df, labels_series, optimize_hyperparameters=args.optimize
        )
