...
from utils.paper_trade_db import record_trade, get_all_trades
...
def simulate_trade_execution(signal, price, confidence):
    ...
    if signal == 'BUY':
        ...
        success_prob = min(0.4 + confidence * 0.3, 0.8)  # Higher confidence = better success rate
...
def run_trading_simulation(duration_minutes=10, trade_interval=30):
    ...
    while datetime.now() < end_time:
        try:
            ...
            # Get RL prediction
            signal, confidence = ensemble_strategy.generate_signal('BTCUSDT', market_data)
            
            # Log the prediction
            logger.info(f"RL Signal: {signal} (confidence: {confidence:.3f}) at price ${current_price:.2f}")
            
            # Only execute trades on BUY/SELL signals
            if signal in ['BUY', 'SELL']:
                # Simulate trade execution
                trade_result = simulate_trade_execution(signal, current_price, confidence)
                ...
                logger.info(f"Trade executed: {signal} at ${current_price:.2f} -> PnL: ${trade_result['pnl']:.2f}, Fee: ${trade_result['fees']:.2f}")
    ...
    win_rate = (simulation_results['profitable_trades'] / simulation_results['total_trades']) * 100 if simulation_results['total_trades'] > 0 else 0
...
def main():
    ...
    try:
        ...
        logger.info(f"The RL model has been updated with {results['total_trades']} new trading experiences")
