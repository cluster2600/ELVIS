    ...
    def __init__(
        ...
        try:
            ...
            if self.password:  # Only add password if it's not None/empty
                ...
                logger.debug(
                    f"Connecting to Redis with authentication at {self.host}:{self.port}"
                )
            else:
                logger.debug(
                    f"Connecting to Redis without authentication at {self.host}:{self.port}"
                )
            ...
            auth_info = (
                "with authentication" if self.password else "without authentication"
            )
