...
class VaultClient:
    ...
    def __init__(
        ...
        self.vault_url = vault_url or os.getenv(
            "VAULT_ADDR", os.getenv("VAULT_URL", "http://localhost:8200")
        )
    ...
    def _init_cache_encryption(self):
        ...
        try:
            ...
            if not key:
                ...
                self.logger.warning(
                    "Generated new cache encryption key. Set VAULT_CACHE_KEY env var to persist across restarts."
                )
    ...
    def _invalidate_cache(self, secret_path: str):
        ...
        keys_to_remove = [
            key for key in self._secret_cache.keys() if key.startswith(secret_path)
        ]
