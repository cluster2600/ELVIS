...
class APIConnectionTester:
            ...
            try:
                ...
                if client.is_authenticated():
                    ...
                    return APIStatus(
                        ...
                        response_time=response_time,
                        last_checked=datetime.now(),
                        details={"url": os.getenv("VAULT_ADDR"), "authenticated": True},
                    )
                else:
                    response_time = time.time() - start_time
                    return APIStatus(
    ...
    def get_overall_health(self) -> Dict[str, Any]:
        ...
        error = sum(
            1 for status in valid_statuses if status.status == ConnectionStatus.ERROR
        )
        disconnected = sum(
            1
            for status in valid_statuses
        ...
        return {
            ...
            "critical_services": [
                name
                for name, status in self.api_statuses.items()
                if status
                and hasattr(status, "status")
                and status.status != ConnectionStatus.CONNECTED
                and name in ["binance_spot", "binance_futures", "postgres"]
            ],
        }
