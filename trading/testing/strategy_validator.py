...
class StrategyValidator:
    ...
    def run_monte_carlo_simulation(
        ...
        for _ in tqdm(
            range(self.mc_config.n_simulations), desc="Running Monte Carlo simulations"
        ):
    ...
    def run_stress_tests(
        ...
        for scenario in tqdm(
            self.mc_config.stress_test_scenarios, desc="Running stress tests"
        ):
            ...
            results[scenario] = {
                ...
                "survival": strategy_results[
                    "survival"
                ],  # Whether strategy survived the stress test
            }

        return results

    def run_walk_forward_analysis(
        self, strategy: callable, data: pd.DataFrame, params: Dict
    ) -> Dict:
        """
        ...
        tscv = TimeSeriesSplit(
            n_splits=self.wf_config.n_splits, test_size=self.wf_config.test_size
        )

        results = {"train_metrics": [], "test_metrics": [], "optimized_params": []}

        for train_idx, test_idx in tqdm(
            tscv.split(data), desc="Running walk-forward analysis"
        ):
    ...
    def run_statistical_tests(
        ...
        for test in self.stats_config.tests:
            if test == "sharpe_ratio":
                ...
                results[test] = self._calculate_calmar_ratio(
                    returns, strategy_results["max_drawdown"]
                )
            elif test == "var":
    ...
    def _simulate_market_data(
        self, data: pd.DataFrame, market_condition: str
    ) -> pd.DataFrame:
    ...
    def _simulate_stress_scenario(
        self, data: pd.DataFrame, scenario: str
    ) -> pd.DataFrame:
    ...
    def _optimize_parameters_regime_based(
        self, strategy: callable, data: pd.DataFrame, params: Dict
    ) -> Dict:
    ...
    def _calculate_confidence_intervals(self, results: Dict) -> Dict:
        ...
        for metric, values in results.items():
            ...
            intervals[metric] = {"lower": mean - margin, "upper": mean + margin}
        return intervals
