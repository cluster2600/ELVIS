...
def setup_event_handlers(secret_key: str):
    ...
    def handle_authenticate(user_payload, data):
        ...
        emit(
            "authenticated",
            {"user": user_payload.get("user"), "message": "Authentication successful"},
        )
    ...
    while True:
        try:
            if socketio and len(connected_clients) > 0:
                ...
                socketio.emit(
                    "performance_update", performance_data, room="performance"
                )
...
def get_recent_trades() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        recent_trades = sorted(
            trades, key=lambda x: x.get("timestamp", ""), reverse=True
        )[:5]
...
def broadcast_trade_execution(trade_data: Dict[str, Any]):
    ...
    if socketio:
        ...
        logger.info(
            f"Trade execution broadcasted: {trade_data.get('symbol')} - {trade_data.get('side')}"
        )
