...
class CoinbaseExecutor(BaseExecutor):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if self.api_key and self.api_secret and self.passphrase:
                ...
                self.logger.info(
                    f"✓ Coinbase authenticated connection established ({env_type})"
                )
            else:
        ...
        try:
            ...
            if price:
                order = self.client.create_limit_buy_order(
                    coinbase_symbol, quantity, price
                )
                order_type = "limit"
            else:
            ...
            self.logger.info(
                f"Coinbase BUY executed: {symbol} {quantity} @ {price or 'market'}"
            )
            return result
    ...
    def execute_sell(
        ...
        try:
            ...
            self.logger.info(
                f"Coinbase SELL executed: {symbol} {quantity} @ {price or 'market'}"
            )
            return result
    ...
    def execute_stop_loss(
        ...
        try:
            ...
            self.logger.info(
                f"Coinbase STOP LOSS set: {symbol} {quantity} @ {stop_price}"
            )
            return result
    ...
    def execute_take_profit(
        self, symbol: str, quantity: float, tp_price: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            order = self.client.create_limit_sell_order(
                coinbase_symbol, quantity, tp_price
            )
            ...
            self.logger.info(
                f"Coinbase TAKE PROFIT set: {symbol} {quantity} @ {tp_price}"
            )
            return result
    ...
    def get_exchange_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fees": {"trading": {"maker": 0.005, "taker": 0.005}},  # 0.5%  # 0.5%
            "limits": {"amount": {"min": 0.001, "max": 1000000}},
