...
class EnhancedBinanceExecutor(BinanceExecutor):
    ...
    def __init__(self, logger: logging.Logger = None, enable_bnb_fees: bool = True, 
                 bnb_trading_enabled: bool = True, min_bnb_balance: float = 0.1, **kwargs):
    ...
    def _setup_bnb_fee_payment(self):
        ...
        try:
            if hasattr(self.client, 'toggle_bnb_burn'):
                ...
                self.logger.info("ℹ️ BNB fee payment setting not available for this client type")
        except Exception as e:
    ...
    def get_bnb_price(self, force_refresh: bool = False) -> float:
        ...
        if (not force_refresh and self._bnb_price_cache and 
            current_time - self._bnb_price_cache_time < self.cache_ttl):
    ...
    def get_bnb_balance(self, force_refresh: bool = False) -> float:
        ...
        if (not force_refresh and self._bnb_balance_cache and 
            current_time - self._bnb_balance_cache_time < self.cache_ttl):
    ...
    def calculate_fee_with_bnb(self, trade_value: float, is_maker: bool = False, 
                              is_futures: bool = True) -> Dict[str, float]:
    ...
    def buy_bnb_for_fees(self, amount_bnb: Optional[float] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get('success'):
                ...
                return {'success': False, 'error': f"BNB purchase failed: {result.get('error')}"}
                
        except Exception as e:
            self.logger.error(f"Error buying BNB: {e}")
            return {'success': False, 'error': str(e)}
    
    def execute_trade_with_bnb_optimization(self, symbol: str, side: str, quantity: float, 
                                          trade_type: str = 'MARKET', **kwargs) -> Dict[str, Any]:
        ...
        if self.enable_bnb_fees and self.bnb_trading_enabled:
            ...
            if bnb_check['should_buy'] and bnb_check['buy_cost_usdt'] < trade_value * 0.1:  # Max 10% of trade value
                self.logger.info(f"🔄 Auto-buying BNB for fee optimization...")
                bnb_result = self.buy_bnb_for_fees()
                
                if bnb_result['success']:
                    self.logger.info(f"✅ BNB purchased: {bnb_result['amount_bnb']:.6f} BNB")
                else:
                    self.logger.warning(f"⚠️ BNB auto-purchase failed: {bnb_result['error']}")
        
        # Calculate expected fees
        fee_analysis = self.calculate_fee_with_bnb(trade_value, is_futures=self.use_futures)
        ...
        if result.get('success'):
            ...
            self.logger.info(f"   Expected fee savings: ${fee_analysis['savings_usdt']:.6f}")
            self.logger.info(f"   BNB discount: {fee_analysis['discount_percent']:.1f}%")
    ...
    def get_enhanced_balance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'fee_optimization': {
                ...
                'futures_discount': self.fee_rates['futures']['taker']['bnb_discount'] * 100,
...
def test_enhanced_executor():
    ...
    logger.info(f"Fee optimization enabled: {balance_info['fee_optimization']['enabled']}")
    ...
    logger.info(f"Savings: ${fee_analysis['savings_usdt']:.6f} ({fee_analysis['discount_percent']:.1f}%)")
    ...
    if bnb_analysis['should_buy']:
        logger.info(f"Buy amount: {bnb_analysis['buy_amount_bnb']:.6f} BNB (${bnb_analysis['buy_cost_usdt']:.2f})")
