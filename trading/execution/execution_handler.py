...
class MockExecutor:
    ...
    def buy(self, symbol: str, quantity: float, leverage: int = 1):
        self.logger.info(f"[MOCK BUY] {symbol} - Quantity: {quantity}, Leverage: {leverage}")
        return {"status": "mock_buy_executed", "symbol": symbol, "quantity": quantity, "leverage": leverage}

    def sell(self, symbol: str, quantity: float, leverage: int = 1):
        self.logger.info(f"[MOCK SELL] {symbol} - Quantity: {quantity}, Leverage: {leverage}")
        return {"status": "mock_sell_executed", "symbol": symbol, "quantity": quantity, "leverage": leverage}
...
class ExecutionHandler:
    ...
    def __init__(self, mode: str, logger: logging.Logger, api_key: str = None, api_secret: str = None):
        """
        ...
        if self.mode == "real":
            ...
            self.executor = BinanceExecutor(logger=self.logger, api_key=api_key, api_secret=api_secret)
