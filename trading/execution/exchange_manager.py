...
class ExchangeManager:
    ...
    def add_exchange(self, name: str, executor_class, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Initialize executor
            executor = executor_class(logger=self.logger, **config)
    ...
    def check_exchange_health(self, name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                hasattr(executor, "is_testnet")
                and executor.is_testnet
                and getattr(executor, "client", None) is None
            ):
            ...
            self.exchange_health[name] = {
                "status": "unhealthy",
                "last_check": datetime.now(),
                "error_count": self.exchange_health.get(name, {}).get("error_count", 0)
                + 1,
                "last_error": str(e),
            }
    ...
    def get_prices_all_exchanges(self, symbol: str) -> Dict[str, float]:
        ...
        def get_price(exchange_name: str, executor: BaseExecutor) -> Tuple[str, float]:
            try:
                if (
                    self.exchange_health.get(exchange_name, {}).get("status")
                    != "healthy"
                ):
                ...
                if (
                    ...
                    mock_price = (
                        executor._get_mock_price(symbol)
                        if hasattr(executor, "_get_mock_price")
                        else 97000.0
                    )
        ...
        with self.price_lock:
            self.price_cache[symbol] = {"prices": prices, "timestamp": datetime.now()}
    ...
    def execute_arbitrage(
        self, opportunity: Dict[str, Any], amount: float
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if arbitrage_result["success"]:
                ...
                self.logger.info(
                    f"Arbitrage executed: {symbol} - Expected profit: ${expected_profit:.2f}"
                )
            else:
        ...
        for symbol in symbols:
            ...
            if prices:
                summary[symbol] = {
                    ...
                    "spread_pct": (max(prices.values()) - min(prices.values()))
                    / min(prices.values())
                    * 100,
        ...
        for name, executor in self.exchanges.items():
            try:
                info[name] = {
                    ...
                    "config": {
                        k: v
                        for k, v in self.exchange_configs.get(name, {}).items()
