...
from .base_order import BaseOrder, OrderSide, OrderStatus, OrderType, TimeInForce
...
class TWAPOrder(BaseOrder):
    ...
    def _generate_slice_schedule(self) -> List[Dict[str, Any]]:
        ...
        for i in range(total_slices):
            ...
            if self.aggressive_on_close and i >= total_slices * 0.8:
                urgency_factor = (
                    1 + (i - total_slices * 0.8) / (total_slices * 0.2) * 0.5
                )
            ...
            schedule.append(
                {
                    ...
                    "urgency_factor": (
                        1.0
                        if not self.aggressive_on_close or i < total_slices * 0.8
    ...
    def get_current_slice(self) -> Optional[Dict[str, Any]]:
        ...
        for i, slice_info in enumerate(
            self.slice_schedule[self.slices_executed :], self.slices_executed
        ):
    ...
    def update_market_data(self, market_data: Dict[str, Any]):
        ...
        self.market_data_history = [
            data for data in self.market_data_history if data["timestamp"] > cutoff_time
        ]
        ...
        self.vwap_tracking.append(
            {
                ...
                "cumulative_value": sum(
                    t["quantity"] * t["price"] for t in self.vwap_tracking
                )
                + quantity * price,
            }
        )
    ...
    def get_twap_progress(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "quantity_progress_percentage": (self.filled_quantity / self.quantity)
            * 100,
            "current_vwap": self.get_current_vwap(),
            "estimated_completion": self.end_time.isoformat(),
            "is_behind_schedule": self.slices_executed
            < (time_progress / 100) * len(self.slice_schedule),
    ...
    def __str__(self) -> str:
        ...
        return (
            f"TWAP {self.side.value.upper()} {self.quantity} {self.symbol} "
...
class TWAPOrderManager:
    ...
    async def submit_twap_order(self, twap_order: TWAPOrder) -> Dict[str, Any]:
        ...
        try:
            ...
            if not exchange:
                return {
                    "success": False,
                    "error": f"Exchange {twap_order.exchange} not available",
                }
    ...
    async def _process_twap_order(self, twap_order: TWAPOrder):
        ...
        try:
            ...
            now = datetime.now()
            if (
                twap_order.status == OrderStatus.FILLED
                or now > twap_order.end_time
                or twap_order.slices_executed >= len(twap_order.slice_schedule)
            ):
            ...
            if twap_order.can_execute_slice():
                ...
                if exchange:
                    ...
                    if not result["success"]:
                        self.logger.warning(
                            f"Failed to submit slice for {twap_order.id}: {result['error']}"
                        )
