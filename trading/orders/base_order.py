...
class BaseOrder(ABC):
    ...
    def update_status(self, status: OrderStatus, message: str = None):
        ...
        if self.logger:
            self.logger.info(f"Order {self.id} status: {old_status.value} -> {status.value}")
    
    def add_fill(self, quantity: float, price: float, fee: float = 0.0, 
                 fill_id: str = None, timestamp: datetime = None):
        """Add a fill record"""
        fill = {
    ...
    def __str__(self) -> str:
        ...
        return (f"{self.order_type.value.upper()} {self.side.value.upper()} "
