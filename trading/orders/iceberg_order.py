...
class IcebergOrder(BaseOrder):
    ...
    def _calculate_total_chunks(self) -> int:
        ...
        return max(1, int(self.quantity / self.chunk_size)) + (
            1 if self.quantity % self.chunk_size > 0 else 0
        )
    ...
    def on_chunk_submitted(self, exchange_order_id: str, chunk_quantity: float):
        ...
        if self.delay_between_chunks > 0:
            self.next_chunk_time = datetime.now() + timedelta(
                seconds=self.delay_between_chunks
            )
    ...
    def _estimate_completion_time(self) -> Optional[str]:
        ...
        ).total_seconds() / self.chunks_filled
    ...
    def __str__(self) -> str:
        ...
        return (
            f"ICEBERG {self.side.value.upper()} {self.quantity} {self.symbol} "
...
class IcebergOrderManager:
    ...
    async def submit_iceberg_order(self, iceberg_order: IcebergOrder) -> Dict[str, Any]:
        ...
        try:
            ...
            if not exchange:
                return {
                    "success": False,
                    "error": f"Exchange {iceberg_order.exchange} not available",
                }
    ...
    async def _submit_next_chunk(
        self, iceberg_order: IcebergOrder, exchange
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if chunk_order_id:
                ...
                iceberg_order.on_chunk_submitted(
                    chunk_order_id, chunk_params["quantity"]
                )
    ...
    async def _monitor_iceberg_orders(self):
        ...
        while self.active_iceberg_orders:
            try:
                for iceberg_id, iceberg_order in list(
                    self.active_iceberg_orders.items()
                ):
            ...
            if iceberg_order.can_submit_next_chunk():
                ...
                if exchange:
                    ...
                    if not result["success"]:
                        self.logger.warning(
                            f"Failed to submit next chunk for {iceberg_order.id}: {result['error']}"
                        )
