...
class OCOOrder(BaseOrder):
    ...
    def on_stop_filled(self, fill_data: Dict[str, Any]):
        ...
        if self.logger:
            self.logger.info(
                f"OCO {self.id}: Stop order filled at {fill_data.get('price')}"
            )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"OCO {self.side.value.upper()} {self.quantity} {self.symbol} "
...
class OCOOrderManager:
    ...
    async def submit_oco_order(self, oco_order: OCOOrder) -> Dict[str, Any]:
        ...
        try:
            ...
            if not exchange:
                return {
                    "success": False,
                    "error": f"Exchange {oco_order.exchange} not available",
                }
    ...
    async def _submit_simulated_oco(
        self, exchange, oco_order: OCOOrder
    ) -> Dict[str, Any]:
