    ...
    async def submit_order(self, order: BaseOrder) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                self.logger.info(
                    f"Advanced order submitted: {order_type.upper()} {order.id}"
                )
    ...
    async def cancel_order(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                self.logger.info(
                    f"Advanced order cancelled: {order_type.upper()} {order_id}"
                )
        ...
        try:
            ...
            if status in ["filled", "cancelled"]:
                ...
                for order_id, order in list(self.all_active_orders.items()):
                    if (
                        (
                            hasattr(order, "exchange_order_id")
                            and order.exchange_order_id == exchange_order_id
                        )
                        or (
                            hasattr(order, "limit_order_id")
                            and order.limit_order_id == exchange_order_id
                        )
                        or (
                            hasattr(order, "stop_order_id")
                            and order.stop_order_id == exchange_order_id
                        )
                        or (
                            hasattr(order, "active_chunk_id")
                            and order.active_chunk_id == exchange_order_id
                        )
                        or (
                            hasattr(order, "active_slice_id")
                            and order.active_slice_id == exchange_order_id
                        )
                    ):
