...
class SentimentStrategy(BaseStrategy):
    ...
    def __init__(self, logger: logging.Logger, **kwargs):
        ...
        # Strategy parameters
        self.sentiment_threshold_positive = kwargs.get(
            "sentiment_threshold_positive", 0.6
        )
        self.sentiment_threshold_negative = kwargs.get(
    ...
    def _get_news_sentiment(self, symbol: str) -> float:
        ...
        try:
            ...
            if (
                cache_key in self.sentiment_cache
                and cache_key in self.sentiment_cache_expiry
                and current_time < self.sentiment_cache_expiry[cache_key]
            ):
    ...
    def _get_technical_signal(self, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            if not all(indicator in latest for indicator in required_indicators):
                self.logger.warning(
                    f"Missing required indicators: {[ind for ind in required_indicators if ind not in latest]}"
                )
                return 0.0
            ...
            self.logger.info(
                f"Technical signal: {technical_signal:.2f} (RSI: {rsi:.2f}, MACD: {macd:.2f}, Signal: {macdsignal:.2f})"
            )
    ...
    def generate_signals(self, data: pd.DataFrame) -> Tuple[bool, bool]:
        ...
        try:
            ...
            self.logger.info(
                f"Combined signal: {combined_signal:.2f} (Sentiment: {sentiment_signal:.2f}, Technical: {technical_signal:.2f})"
            )
            ...
            self.logger.info(
                f"Signal Check: Combined={combined_signal:.2f}, Buy={buy_signal}, Sell={sell_signal}"
            )
