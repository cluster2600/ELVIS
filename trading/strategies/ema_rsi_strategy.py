...
try:
    ...
    HAS_TALIB = False
    print(
        "Warning: talib not available, EMA RSI strategy will use fallback calculations"
    )
...
class EmaRsiStrategy(BaseStrategy):
    ...
    def __init__(self, logger: logging.Logger, **kwargs):
        ...
        self.stop_loss_pct = kwargs.get(
            "stop_loss_pct", TRADING_CONFIG["STOP_LOSS_PCT"]
        )
        self.take_profit_pct = kwargs.get(
            "take_profit_pct", TRADING_CONFIG["TAKE_PROFIT_PCT"]
        )
        self.position_size_pct = kwargs.get(
            "position_size_pct", 0.5
        )  # 50% of available capital
        ...
        try:
            ...
            if "rsi" not in df.columns:
                if len(df) >= self.rsi_period:
                    df.loc[:, "rsi"] = talib.RSI(
                        df["close"].values, timeperiod=self.rsi_period
                    )
                else:
            ...
            if "volatility" not in df.columns:
                ...
                if len(df) >= 20:
                    df.loc[:, "volatility"] = df["close"].pct_change().rolling(
                        20
                    ).std() * np.sqrt(252)
                else:
            ...
            if (
                pd.isna(latest["ema_short"])
                or pd.isna(latest["ema_long"])
                or pd.isna(latest["rsi"])
                or pd.isna(latest["volatility"])
            ):
            ...
            buy_signal = (
                ...
                and latest["volatility"] > self.volatility_threshold
            )
            ...
            self.logger.info(
                f"Signal Check: EMA9={latest['ema_short']:.2f}, EMA21={latest['ema_long']:.2f}, RSI={latest['rsi']:.2f}, Volatility={latest['volatility']:.4f}, Buy={buy_signal}, Sell={sell_signal}"
            )
    ...
    def calculate_position_size(
        ...
        try:
            ...
            if quantity < min_quantity:
                self.logger.warning(
                    f"Calculated quantity {quantity:.8f} is below minimum {min_quantity}. Using minimum."
                )
                quantity = min_quantity

            self.logger.info(
                f"Calculated position size: {quantity:.8f} BTC (value: {quantity * current_price:.2f} USD)"
            )
    ...
    def calculate_stop_loss(self, data: pd.DataFrame, entry_price: float) -> float:
        ...
        try:
            ...
            self.logger.info(
                f"Calculated stop loss: {stop_loss:.2f} USD (entry: {entry_price:.2f} USD)"
            )
    ...
    def calculate_take_profit(self, data: pd.DataFrame, entry_price: float) -> float:
        ...
        try:
            ...
            self.logger.info(
                f"Calculated take profit: {take_profit:.2f} USD (entry: {entry_price:.2f} USD)"
            )
