...
from utils.paper_trade_db import get_open_positions, add_open_position, record_trade, close_position
import requests
...
class BalancedStarterStrategy:
    ...
    def open_initial_balanced_positions(self, current_price: float, available_capital: float):
        """Open initial balanced LONG and SHORT positions"""
        if self.initial_positions_opened:
            return
            
        self.logger.info(f"🎯 OPENING INITIAL BALANCED POSITIONS at ${current_price:,.2f}")
        ...
        target_notional_usd = 1000.0  # $1000 notional per position (enough for real profits)
        target_quantity = target_notional_usd / current_price  # Convert USD to BTC
        
        # Scale with leverage and ensure profitable size
        position_size = max(target_quantity, 0.0008)  # Minimum 0.0008 BTC (~$95 at current prices)
        
        self.logger.info(f"💰 PROFITABLE SCALPING SETUP: Targeting ${self.target_profit_per_trade} profit per trade")
        self.logger.info(f"📊 Position size: {position_size:.6f} BTC (${position_size * current_price:.2f} notional)")
        self.logger.info(f"⚡ Expected price move: ${expected_price_move} for target profit (PROFITABLE!)")
        ...
        try:
            ...
            for i in range(self.target_long_positions):
                ...
                if i == 0:
                    self.logger.info(f"✅ Opening {self.target_long_positions} LONG positions @ ${current_price:,.2f}")
            ...
            for i in range(self.target_short_positions):
                ...
                # Reduced logging - only log first position
                if i == 0:
                    self.logger.info(f"✅ Opening {self.target_short_positions} SHORT positions @ ${current_price:,.2f}")
            
            self.initial_positions_opened = True
            self.logger.info(f"🎯 BALANCED STARTUP COMPLETE: {self.target_long_positions} LONG + {self.target_short_positions} SHORT positions")
    ...
    def calculate_market_bias(self, data: pd.DataFrame, current_price: float) -> float:
        ...
        try:
            ...
            if len(data) < 5:
                self.logger.warning(f"Insufficient data for bias calculation: {len(data)} rows")
                # At least calculate level bias which doesn't need historical data
            else:
                self.logger.debug(f"Calculating market bias with {len(data)} data points")
            
            # 1. Price momentum (20%) - only if we have enough data
            if len(data) >= 5:
                try:
                    # Clean close prices for momentum calculation
                    clean_closes = pd.to_numeric(data['close'], errors='coerce').dropna()
                    if len(clean_closes) >= 5:
                        ...
                        momentum_bias = (price_change_5m * 0.7 + price_change_15m * 0.3) * 10  # Scale to -1 to +1
                        bias_score += momentum_bias * 0.2
                    else:
                        self.logger.warning(f"Insufficient clean data for momentum: {len(clean_closes)}")
            ...
            if 'volume' in data.columns and len(data) >= 10:
                try:
                    # Clean volume data
                    clean_volume = pd.to_numeric(data['volume'], errors='coerce').dropna()
                    if len(clean_volume) >= 10:
                        recent_volume = clean_volume.iloc[-5:].mean()
                        avg_volume = clean_volume.iloc[-min(20, len(clean_volume)):].mean()
                        
                        if avg_volume > 0:
                            volume_bias = (recent_volume - avg_volume) / avg_volume
                            bias_score += np.clip(volume_bias, -0.5, 0.5) * 0.15
                        else:
                            self.logger.warning("Average volume is zero, skipping volume bias")
                    else:
                        self.logger.warning(f"Insufficient clean volume data: {len(clean_volume)}")
            ...
            if len(data) >= 14:
                try:
                    ...
                    if len(close_prices) >= 14:
                        ...
                        self.logger.warning(f"Insufficient clean close price data for RSI: {len(close_prices)}")
            ...
            if abs(total_performance) < 5.0:  # Only if total loss < $5
                ...
                self.logger.warning(f"🚨 Ignoring performance bias due to extreme losses: ${total_performance:.2f}")
            ...
            if len(data) >= 5:
                recent_high = data['high'].tail(5).max() if 'high' in data.columns else current_price
                price_momentum = (current_price - data['close'].iloc[-5]) / data['close'].iloc[-5] if len(data) >= 5 else 0
                
                # Strong upward momentum at new highs = AVOID SHORTS
                if current_price > recent_high * 1.002 and price_momentum > 0.01:  # Breaking to new highs with momentum
                    level_bias = 0.5  # BULLISH bias during breakouts
                    self.logger.info(f"🚀 ATH BREAKOUT: ${current_price:,.2f} breaking higher with {price_momentum*100:.2f}% momentum - AVOID SHORTS")
                elif current_price > 125000 and price_momentum > 0.005:  # High price with momentum - NEW PARADIGM
                    level_bias = 0.2  # Slight bullish bias
                    self.logger.info(f"📈 HIGH MOMENTUM: ${current_price:,.2f} with momentum - REDUCED SHORT BIAS")
                elif current_price > 123000:
                    level_bias = -0.3  # Reduced bearish bias (was -0.8) - NEW PARADIGM
                    self.logger.info(f"⚠️ HIGH LEVEL: ${current_price:,.2f} > $123,000 - CAUTIOUS BEARISH")
            ...
            if current_price > 125000:  # Log detailed bias during high prices - NEW PARADIGM
                ...
                self.logger.info(f"   Performance: {perf_bias:.3f} (weight: 0.1 - REDUCED)")
            ...
            elif abs(market_bias) > 0.2 or (hasattr(self, 'last_logged_bias') and abs(market_bias - self.last_logged_bias) > 0.1):
                self.logger.info(f"📊 Market Bias: {market_bias:.3f} (Level: {level_bias:.3f})")
    ...
    def adapt_positions(self, current_price: float, available_capital: float, market_bias: float):
        ...
        try:
            ...
            if price_above_125k and strong_upward_momentum:
                ...
                self.logger.error(f"🚨 ATH PROTECTION: BTC ${current_price:,.0f} - ONLY LONG POSITIONS")
                ...
                target_long = 0
                target_short = self.target_short_positions  # SHORT only during strong bear
            ...
            total_to_add = max(0, target_long - current_long) + max(0, target_short - current_short)
            if total_to_add > 0:
                self.logger.info(f"🔄 ADAPTING: {current_long}→{target_long} LONG, {current_short}→{target_short} SHORT")
            ...
            target_notional_usd = 1000.0  # $1000 notional per position (optimized for $5.00 profits)
            target_quantity = target_notional_usd / current_price  # Convert USD to BTC
            
            # Smart sizing based on portfolio health
            if available_capital < 500.0:  # Portfolio damaged
                # Conservative scaling for recovery
                size_reduction = max(0.3, available_capital / 1000.0)  # Minimum 70% reduction
                target_quantity *= size_reduction
                self.logger.warning(f"🔧 RECOVERY MODE: Portfolio ${available_capital:.2f} - Scaling positions to {size_reduction:.2f}x")
            elif available_capital < 800.0:  # Portfolio partially damaged
                # Moderate scaling for safety
                size_reduction = max(0.6, available_capital / 1000.0)  # Minimum 40% reduction
                target_quantity *= size_reduction
                self.logger.info(f"⚠️ CAUTIOUS MODE: Portfolio ${available_capital:.2f} - Scaling positions to {size_reduction:.2f}x")
            
            position_size = max(target_quantity, 0.0008)  # Minimum 0.0008 BTC (~$95 at current prices)
            
            self.logger.info(f"💰 Profitable position size: {position_size:.6f} BTC (${position_size * current_price:.2f} notional)")
        ...
        try:
            ...
            for pos in positions:
                ...
                if target_bias == 'SHORT' and side.upper() == 'BUY':
                    ...
                    self.logger.info(f"🚨 ATH CLOSURE: Closed LONG position {pos_id} - Entry: ${entry_price:,.2f}, Exit: ${current_price:,.2f}, P&L: ${pnl:.2f}")
                ...
                # At strong bullish bias, close SHORT positions  
                    ...
                    self.logger.info(f"📈 BULLISH CLOSURE: Closed SHORT position {pos_id} - Entry: ${entry_price:,.2f}, Exit: ${current_price:,.2f}, P&L: ${pnl:.2f}")
            
            if closed_count > 0:
                self.logger.info(f"🎯 EMERGENCY CLOSURE COMPLETE: Closed {closed_count} conflicting positions")
        ...
        try:
            ...
            for pos in positions:
                ...
                if pnl >= realistic_profit_target:
                    ...
                    self.logger.info(f"💰 PROFIT: {side} ${pnl:.4f} (target: ${realistic_profit_target})")
                ...
                # EMERGENCY: Close ANY position losing more than stop loss
                    ...
                    closed_count += 1
                    self.last_trade_time = current_time  # Track last trade time
                    self.logger.error(f"🛑 STOP LOSS: {side} ${pnl:.4f} (target: ${realistic_stop_loss})")
                ...
                # OPTIMIZED: Force close positions with reasonable losses (safety net)
                    ...
                    closed_count += 1
                    self.last_trade_time = current_time  # Track last trade time
                    self.logger.error(f"🚨 EMERGENCY STOP: {side} ${pnl:.4f} - FORCE CLOSING RUNAWAY LOSS")
        ...
        try:
            ...
            for pos in positions:
                ...
                if pnl < -0.05:  # Close anything losing more than 5 cents
                    close_position(pos_id, current_price, pnl)
                    closed_count += 1
                    self.logger.error(f"🚨 EMERGENCY CLOSE: {side} position {pos_id} - Loss: ${pnl:.2f}")
            
            if closed_count > 0:
                self.logger.error(f"🚨 EMERGENCY COMPLETE: Closed {closed_count} losing positions to protect portfolio")
    ...
    def execute_strategy(self, data: pd.DataFrame, available_capital: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if available_capital < 200.0:  # If portfolio dropped below $200
                self.logger.error(f"🚨 EMERGENCY: Portfolio at ${available_capital:.2f} - CLOSING ALL LOSING POSITIONS")
                self.emergency_close_losing_positions(current_price)
                return {'action': 'EMERGENCY_PROTECTION', 'reason': 'Portfolio protection activated'}
            ...
            if current_price > 123000:
                self.logger.info(f"🚀 ATH LEVEL: Price ${current_price:,.2f} > $123,000 - NEW PARADIGM")
            ...
            if not self.should_adapt() and abs(self.market_bias) < 0.4:
                return {'action': 'MONITORING', 'price': current_price, 'market_bias': self.market_bias}
    ...
    def generate_signal(self, symbol: str, market_data: Dict[str, Any]) -> Tuple[str, float]:
