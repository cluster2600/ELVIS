...
class TrendFollowingStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> Tuple[bool, bool]:
        ...
        try:
            ...
            buy_signal = bool(golden_cross and adx > self.adx_threshold)

            # Generate sell signal
            # Death cross or weakening trend
            sell_signal = bool(
                death_cross or (adx < self.adx_threshold and fast_ma < slow_ma)
            )

            self.logger.info(
                f"Signal Check: Fast MA={fast_ma:.2f}, Slow MA={slow_ma:.2f}, ADX={adx:.2f}, Buy={buy_signal}, Sell={sell_signal}"
            )
    ...
    def calculate_position_size(
        ...
        try:
            ...
            if "adx" in data.iloc[-1]:
                ...
                self.logger.info(
                    f"Adjusting position size by trend factor: {trend_factor:.2f} (ADX: {adx:.2f})"
                )
    ...
    def calculate_trailing_stop(
        ...
        try:
            ...
            self.logger.info(
                f"Calculated trailing stop: {stop_price:.2f} USD (entry: {entry_price:.2f} USD, highest: {highest_price:.2f} USD)"
            )
