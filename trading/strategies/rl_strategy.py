...
class RLStrategy(BaseStrategy):
    ...
    def _initial_training(self):
        ...
        try:
            ...
            if success:
                ...
                self.logger.warning("⚠️ Initial RL training failed, model may not perform optimally")
        ...
        for symbol in self.symbols:
            try:
                ...
                if confidence < self.confidence_threshold:
                    self.logger.info(f"RL confidence {confidence:.3f} below threshold {self.confidence_threshold}, using HOLD")
                    signal = "HOLD"
                    confidence = 0.5
                ...
                self.logger.info(f"RL signal for {symbol}: {signal} (confidence: {confidence:.3f})")
    ...
    def generate_signal(self, symbol: str, market_data: dict) -> Tuple[str, float]:
        ...
        try:
            ...
            # Apply confidence threshold
            if confidence < self.confidence_threshold:
                self.logger.info(f"RL confidence {confidence:.3f} below threshold, using HOLD")
                signal = "HOLD"
                confidence = 0.5
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'model_type': 'DQN',
                'state_size': self.rl_model.feature_extractor.state_size if hasattr(self.rl_model.feature_extractor, 'state_size') else 16,
                'action_size': 3,
    ...
    def calculate_position_size(self, data: pd.DataFrame, current_price: float, 
                              available_capital: float, leverage: float = 1.0, 
