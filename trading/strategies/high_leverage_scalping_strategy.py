...
class HighLeverageScalpingStrategy:
    ...
    def __init__(self, logger: logging.Logger):
        ...
        self.logger.info(f"⚡ Target: {self.scalp_profit_target}% moves = {self.scalp_profit_target * 100}% profit @ {self.leverage}x")
        self.logger.info(f"🛑 Stop Loss: {self.stop_loss_pct}% = {self.stop_loss_pct * 100}% loss @ {self.leverage}x")
    
    def analyze_scalping_opportunity(self, data: pd.DataFrame) -> Tuple[str, float, Dict]:
        """
        ...
        try:
            ...
            avg_volume = float(recent_data['volume'].mean()) if 'volume' in recent_data else 1000
            ...
            price_change_1min = ((price - float(previous['close'])) / float(previous['close'])) * 100
            price_change_5min = ((price - float(data.iloc[-6]['close'])) / float(data.iloc[-6]['close'])) * 100 if len(data) > 6 else 0
        ...
        try:
            ...
            if abs(price_change_1min) > self.momentum_threshold:
                if price_change_1min > 0:
                    buy_score += 0.4
                    confidence_factors.append(f"Positive 1min momentum: {price_change_1min:.3f}%")
                else:
                    sell_score += 0.4
                    confidence_factors.append(f"Negative 1min momentum: {price_change_1min:.3f}%")
            ...
            if volume_ratio > self.volume_multiplier:
                ...
                confidence_factors.append(f"Volume confirmation: {volume_ratio:.1f}x average")
            ...
            if signal != "HOLD":
                ...
                self.logger.info(f"🎯 Confidence: {confidence:.1%} | Factors: {', '.join(confidence_factors)}")
                self.logger.info(f"📊 Momentum: {momentum_strength:.3f}% | Volume: {volume_ratio:.1f}x | RSI: {rsi:.1f}")
    ...
    def calculate_scalping_position_size(self, current_price: float, available_capital: float, 
        ...
        try:
            ...
            confidence_multiplier = min(2.0, max(0.5, confidence * 2))  # 0.5x to 2x based on confidence
            ...
            stop_loss_dollars = current_price * (self.stop_loss_pct / 100) * self.leverage
            ...
            self.logger.info(f"💰 SCALP POSITION: {position_size_btc:.6f} BTC (${position_size_btc * current_price:.2f})")
            self.logger.info(f"⚡ Leverage: {self.leverage}x | Risk: {base_risk_pct * confidence_multiplier:.1%} | Confidence: {confidence:.1%}")
    ...
    def calculate_position_size(self, data: pd.DataFrame, current_price: float, 
                              available_capital: float, leverage: float = 100.0, 
                              signal_confidence: float = 0.5) -> float:
        """
        Calculate position size for the strategy
        """
        return self.calculate_scalping_position_size(current_price, available_capital, signal_confidence)
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        if self.recent_signals:
            ...
            avg_confidence = sum(s['confidence'] for s in self.recent_signals) / recent_count
            recent_performance = {
