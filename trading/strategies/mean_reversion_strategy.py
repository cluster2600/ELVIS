...
class MeanReversionStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> Tuple[bool, bool]:
        ...
        try:
            ...
            buy_signal = bool(close < lower_band and rsi < self.rsi_oversold)

            # Generate sell signal
            # Price above upper band or RSI overbought
            sell_signal = bool(close > upper_band or rsi > self.rsi_overbought)

            self.logger.info(
                f"Signal Check: RSI={rsi:.2f}, Close={close:.2f}, Upper={upper_band:.2f}, Lower={lower_band:.2f}, Buy={buy_signal}, Sell={sell_signal}"
            )
