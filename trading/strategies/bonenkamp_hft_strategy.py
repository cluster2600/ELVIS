...
class BonenkampHFTStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        self.social_data_collector = (
            SocialDataCollector(logger) if use_social_features else None
        )
        ...
        self.logger.info(
            f"🎪 Target Sharpe: {self.target_sharpe_ratio}, Return: {self.target_annual_return:.1%}, F1: {self.target_f1_score:.3f}"
        )
    ...
    def train_model(self, training_data: pd.DataFrame) -> float:
        ...
        try:
            ...
            self.logger.info(
                f"🎓 Training Bonenkamp HFT model with {len(training_data)} samples"
            )
            ...
            self.logger.info(
                f"🎯 Model trained: F1={mean_f1:.3f} (target: {self.target_f1_score:.3f})"
            )
        ...
        for symbol in ["BTCUSDT"]:  # Focus on Bitcoin as per research
            ...
            try:
                ...
                if not self.is_trained:
                    ...
                    try:
                        ...
                        confidence_threshold = (
                            0.55  # Higher threshold for quality trades
                        )
                ...
                if hasattr(self.rf_model, "feature_importances_") and self.is_trained:
                    ...
                    self.logger.debug(
                        f"Feature importances: {dict(zip(self._get_feature_names(), importances))}"
                    )

                self.logger.info(
                    f"🎯 Bonenkamp signal for {symbol}: {signal} ({confidence:.3f})"
                )
                signals[symbol] = {
                    "signal": signal,
    ...
    def calculate_position_size(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.info(
                f"Position size: {position_size:.6f} BTC (confidence: {confidence:.3f})"
            )
    ...
    def calculate_stop_loss(
        ...
        try:
            ...
            self.logger.info(
                f"💔 Stop loss calculated: ${stop_loss:.2f} (ATR: {atr:.2f})"
            )
            return stop_loss
    ...
    def calculate_take_profit(
        ...
        try:
            ...
            self.logger.info(
                f"💚 Take profit calculated: ${take_profit:.2f} (ATR: {atr:.2f}, confidence: {confidence:.3f})"
            )
            return take_profit
    ...
    def calculate_performance_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            self.logger.info(
                f"   Sharpe: {sharpe_ratio:.2f} (target: {self.target_sharpe_ratio:.2f})"
            )
            self.logger.info(
                f"   Annual Return: {annual_return:.1%} (target: {self.target_annual_return:.1%})"
            )
            self.logger.info(
                f"   F1-Score: {avg_f1_score:.3f} (target: {self.target_f1_score:.3f})"
            )
...
class SocialDataCollector:
    ...
    def _is_cache_valid(self, key: str) -> bool:
        ...
        return (
            key in self.cache
            and key in self.cache_expiry
            and datetime.now() < self.cache_expiry[key]
        )
