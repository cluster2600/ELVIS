...
class LLMEnhancedStrategy(BaseStrategy):
    ...
    def _load_latest_models(self):
        ...
        try:
            ...
            classifier_pattern = os.path.join(self.model_base_path, "llm_enhanced_classifier_*.joblib")
            ...
            regressor_file = os.path.join(self.model_base_path, f"llm_enhanced_regressor_{timestamp}.joblib")
            scaler_file = os.path.join(self.model_base_path, f"llm_enhanced_scaler_{timestamp}.joblib")
            metadata_file = os.path.join(self.model_base_path, f"training_metadata_{timestamp}.json")
            
            # Load all components
            self.classifier = joblib.load(latest_classifier)
            self.regressor = joblib.load(regressor_file) if os.path.exists(regressor_file) else None
            self.scaler = joblib.load(scaler_file) if os.path.exists(scaler_file) else None
            
            if os.path.exists(metadata_file):
                with open(metadata_file, 'r') as f:
                    self.model_metadata = json.load(f)
                self.llm_features_expected = self.model_metadata.get('data_info', {}).get('llm_features', 0)
            
            self.logger.info(f"✅ Loaded LLM-enhanced models from {timestamp}")
            if self.model_metadata:
                accuracy = self.model_metadata.get('model_performance', {}).get('classification_accuracy', 'unknown')
                mse = self.model_metadata.get('model_performance', {}).get('regression_mse', 'unknown')
    ...
    def _prepare_features(self, data: pd.DataFrame) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        feature_info = {
            ...
            'llm_features': len([col for col in feature_columns if col.startswith('llm_')]),
    ...
    def _add_llm_style_features(self, df: pd.DataFrame):
        ...
        volatility = df['volatility'].iloc[-1] if not pd.isna(df['volatility'].iloc[-1]) else 10
        macd = df['macd'].iloc[-1] if not pd.isna(df['macd'].iloc[-1]) else 0
        
        # Sentiment analysis
        sentiment_factors = []
        if price_change > 1: sentiment_factors.append(0.6 + min(0.3, price_change / 10))
        elif price_change < -1: sentiment_factors.append(0.4 - min(0.3, abs(price_change) / 10))
        else: sentiment_factors.append(0.5)
        
        if rsi > 70: sentiment_factors.append(0.8)
        elif rsi < 30: sentiment_factors.append(0.2)
        elif rsi > 50: sentiment_factors.append(0.6)
        else: sentiment_factors.append(0.4)
        
        if macd > 0: sentiment_factors.append(0.6)
        else: sentiment_factors.append(0.4)
        ...
        df['llm_bullish_probability'] = bullish_indicators / max(1, total_indicators) if total_indicators > 0 else 0.5
        
        # Risk assessment
        risk_factors = [volatility / 30, abs(price_change) / 15]
        if rsi > 80 or rsi < 20: risk_factors.append(0.8)
        df['llm_risk_score'] = min(1.0, sum(risk_factors) / len(risk_factors))
        
        # Volatility prediction
        df['llm_volatility_prediction'] = min(1.0, (volatility / 20) + abs(price_change) / 20)
        
        # Trend strength
        df['llm_trend_strength'] = min(1.0, (abs(price_change) / 5 + abs(macd) * 10 + abs(rsi - 50) / 25) / 3)
        
        # Ensure values are in [0.1, 0.9] range
        llm_columns = ['llm_sentiment_score', 'llm_confidence_score', 'llm_bullish_probability', 
                      'llm_risk_score', 'llm_volatility_prediction', 'llm_trend_strength']
    ...
    def should_buy(self, data: pd.DataFrame, current_price: float, **kwargs) -> bool:
        ...
        try:
            ...
            will_buy = buy_probability[1] > buy_probability[0]  # Index 1 is typically the "positive" class
            ...
            if self.regressor:
                ...
                if (will_buy and price_change_prediction > 0) or (not will_buy and price_change_prediction < 0):
                    buy_confidence *= 1.1
            
            # Log prediction details
            self.logger.info(f"🧠 LLM Prediction: Buy={will_buy}, Confidence={buy_confidence:.3f}")
            self.logger.info(f"   Features: {feature_info['total_features']} total, {feature_info['llm_features']} LLM")
            self.logger.info(f"   Market: Price=${feature_info['latest_price']:.2f}, Change={feature_info['price_change']:.2f}%, Vol={feature_info['volatility']:.2f}%")
            ...
            if should_buy:
                ...
                self.logger.info(f"❌ No buy signal (confidence {buy_confidence:.1%} < {self.min_confidence_threshold:.1%})")
    ...
    def should_sell(self, data: pd.DataFrame, current_price: float, **kwargs) -> bool:
        ...
        try:
            ...
            will_sell = sell_probability[0] > sell_probability[1]  # Index 0 is typically the "negative" class
            
            # Additional confidence check using regressor if available
            if self.regressor:
                price_change_prediction = self.regressor.predict(X_scaled)[0]
                # Increase confidence if regressor agrees with classifier
                if (will_sell and price_change_prediction < 0) or (not will_sell and price_change_prediction > 0):
                    sell_confidence *= 1.1
            
            # Log prediction details
            self.logger.info(f"🧠 LLM Prediction: Sell={will_sell}, Confidence={sell_confidence:.3f}")
            
            # Make decision based on confidence threshold
            should_sell = will_sell and sell_confidence >= self.min_confidence_threshold
            
            if should_sell:
                self.logger.info(f"✅ SELL signal with {sell_confidence:.1%} confidence")
            else:
                self.logger.info(f"❌ No sell signal (confidence {sell_confidence:.1%} < {self.min_confidence_threshold:.1%})")
    ...
    def _traditional_buy_signal(self, data: pd.DataFrame, current_price: float) -> bool:
        ...
        rsi = data['close'].diff().rolling(14).apply(lambda x: 100 - (100 / (1 + x[x > 0].mean() / abs(x[x < 0]).mean())))
    ...
    def _traditional_sell_signal(self, data: pd.DataFrame, current_price: float) -> bool:
    ...
    def get_position_size(self, available_capital: float, current_price: float, **kwargs) -> float:
        ...
        if self.classifier and hasattr(self, '_last_confidence'):
            ...
            confidence_multiplier = min(2.0, self._last_confidence / self.min_confidence_threshold)
        ...
        self.logger.info(f"💰 Position size: ${position_value:.2f} ({position_size:.6f} BTC)")
    ...
    def get_take_profit_price(self, entry_price: float, is_long: bool) -> Optional[float]:
    ...
    def train_model(self, data: pd.DataFrame):
        ...
        self.logger.info("   Consider running train_with_llm.py manually for retraining")
    ...
    def calculate_take_profit(self, data: pd.DataFrame, entry_price: float) -> float:
        ...
        """
        return self.get_take_profit_price(entry_price, is_long=True) or entry_price * 1.03
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        if self.model_metadata:
            info.update({
                'model_accuracy': self.model_metadata.get('model_performance', {}).get('classification_accuracy'),
                'model_mse': self.model_metadata.get('model_performance', {}).get('regression_mse'),
                'training_samples': self.model_metadata.get('data_info', {}).get('total_samples'),
                'total_features': self.model_metadata.get('data_info', {}).get('total_features'),
