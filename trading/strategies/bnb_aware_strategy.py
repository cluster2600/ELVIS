...
class BNBAwareStrategy(BaseStrategy):
    ...
    def __init__(self, logger: logging.Logger, symbols: List[str] = None, **kwargs):
        ...
        self.bnb_allocation_percent = kwargs.get(
            "bnb_allocation_percent", 5.0
        )  # 5% for BNB
    ...
    def _generate_bnb_signal(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            should_accumulate_bnb = (
                bnb_balance < self.min_bnb_balance
                or bnb_allocation_current < self.bnb_allocation_percent
            )

            if should_accumulate_bnb:
                # Calculate how much BNB to buy
                target_bnb_value = total_balance_usdt * (
                    self.bnb_allocation_percent / 100
                )
                ...
                if needed_bnb_value > 10:  # Only if we need more than $10 worth
                    return {
                        ...
                        "confidence": 0.8,
                        "position_size": min(
                            needed_bnb_value / current_price, 0.05
                        ),  # Max 5% position
            ...
            return {
                "action": "HOLD",
                "confidence": 0.0,
                "reason": f"BNB signal error: {e}",
            }

    def _optimize_signal_for_bnb_fees(
        self, signal: Dict[str, Any], symbol: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def get_position_size(
        self, signal: Dict[str, Any], symbol: str, balance: Dict[str, float]
    ) -> float:
        """
    ...
    def validate_trade(
        ...
        if symbol == "BNBUSDT":
            ...
            if signal.get("action") == "SELL" and bnb_balance <= self.min_bnb_balance:
                ...
                validation["reason"] = (
                    f"Cannot sell BNB: balance ({bnb_balance:.6f}) at or below minimum ({self.min_bnb_balance})"
                )
