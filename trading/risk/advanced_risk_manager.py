...
class AdvancedRiskManager:
    def __init__(self, logger: logging.Logger, **kwargs):
        ...
        self.open_positions = []  # List of dicts: [{'symbol': ..., 'entry_price': ..., 'quantity': ..., 'leverage': ...}]
        self.maintenance_margin_rate = 0.005  # 0.5%
        self.liquidation_fee_rate = 0.005  # 0.5%

    def open_position(self, symbol: str, entry_price: float, quantity: float, leverage: float):
        notional_value = entry_price * quantity
        initial_margin = notional_value / leverage
        
        if initial_margin > self.margin_balance:
            self.logger.warning(f"Insufficient margin to open position: Required={initial_margin}, Available={self.margin_balance}")
            return False
        ...
        self.logger.info(f"Opened position: {symbol} {quantity} @ {entry_price}x{leverage} | Used Margin: {self.used_margin:.2f} | Free Margin: {self.margin_balance:.2f}")
        return True
    ...
    def check_liquidation(self, current_price: float):
        for position in self.open_positions[:]:
            ...
            unrealized_pnl = (current_price - position['entry_price']) * position['quantity'] * (1 if position['leverage'] > 0 else -1)
            free_margin = self.margin_balance + unrealized_pnl
            
            if free_margin <= maintenance_margin:
                self.logger.error(f"Liquidation triggered for {position['symbol']}! Free Margin: {free_margin:.2f} <= Maintenance: {maintenance_margin:.2f}")
                ...
                self.logger.info(f"Position liquidated. Liquidation Fee: {liquidation_fee:.2f} | New Balance: {self.margin_balance:.2f}")

    def close_position(self, symbol: str, exit_price: float):
        for position in self.open_positions:
            if position['symbol'] == symbol:
                notional = position['entry_price'] * position['quantity']
                pnl = (exit_price - position['entry_price']) * position['quantity'] * (1 if position['leverage'] > 0 else -1)
                self.margin_balance += (notional / position['leverage']) + pnl
                self.used_margin -= notional / position['leverage']
                self.logger.info(f"Closed position {symbol} @ {exit_price} | PnL: {pnl:.2f} | Balance: {self.margin_balance:.2f}")
                self.open_positions.remove(position)
                return
