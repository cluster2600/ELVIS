...
class LLMTradingAdvisor:
    ...
    def _make_llm_request(
        ...
        approaches = [
            ...
            {"headers": {"Content-Type": "application/json"}, "name": "Simple format"},
        ...
        for approach in approaches:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if content:
                        self.logger.debug(
                            f"✅ LLM request successful with {approach['name']}"
                        )
                        return content
                else:
                    self.logger.debug(
                        f"❌ {approach['name']} failed: {response.status_code}"
                    )
        ...
        self.logger.error(
            f"All LLM request approaches failed for endpoint: {self.chat_endpoint}"
        )
        return None

    def analyze_market_sentiment(
        self, market_data: Dict[str, Any], recent_news: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
...
- Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
            ...
            if response:
                ...
                try:
                    ...
                    if json_start != -1 and json_end > json_start:
                        ...
                        self.logger.info(
                            f"🧠 LLM Analysis: {result.get('sentiment', 'Unknown')} sentiment with {result.get('confidence', 0):.1%} confidence"
                        )
                        return result
    ...
    def enhance_trading_signal(
        self, current_signal: str, signal_confidence: float, market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
...
Please validate this signal."""
                    ...
                    if json_start != -1 and json_end > json_start:
                        ...
                        adjusted_confidence = max(
                            ...
                            min(
                                1.0,
                                signal_confidence
                                + result.get("confidence_adjustment", 0),
                            ),
                        )
                        ...
                        self.logger.info(
                            f"🧠 LLM Signal Validation: {validation} - Confidence {signal_confidence:.1%} → {adjusted_confidence:.1%}"
                        )
    ...
    def generate_market_report(
        self, market_data: Dict[str, Any], recent_trades: List[Dict] = None
    ) -> str:
        """
...
- Volume: {market_data.get('volume', 0):,.0f}"""
    ...
    def analyze_trading_performance(
        ...
        try:
            ...
            avg_win = sum(float(t.get("pnl", 0)) for t in profitable_trades) / max(
                len(profitable_trades), 1
            )
            avg_loss = sum(float(t.get("pnl", 0)) for t in losing_trades) / max(
                len(losing_trades), 1
            )
            ...
            if analysis:
                ...
                performance_score = min(
                    10, max(1, 5 + (win_rate - 50) / 10 + (total_pnl / 100))
                )

                # Determine risk level
                risk_level = (
                    "HIGH"
                    if total_pnl < -50 or win_rate < 30
                    else "MEDIUM" if total_pnl < 0 or win_rate < 45 else "LOW"
                )
    ...
    def analyze_market_trends(
        self, price_data: pd.DataFrame, volume_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            price_24h_ago = (
                float(price_data["close"].iloc[-24])
            ...
            ma_20 = (
                price_data["close"].rolling(20).mean().iloc[-1]
                if len(price_data) >= 20
                else current_price
            )
            ma_50 = (
                price_data["close"].rolling(50).mean().iloc[-1]
            ...
            volatility = (
                price_data["close"].pct_change().std() * 100
                if len(price_data) > 1
                else 0
            )
            ...
            if analysis:
                return {
                    ...
                    "momentum": (
                        ...
                        else "MODERATE" if abs(price_change_24h) > 1 else "WEAK"
                    ),
                }
