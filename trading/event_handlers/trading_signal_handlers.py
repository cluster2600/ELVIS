...
def validate_trading_signal(event: TradingSignalEvent):
    ...
    try:
        ...
        logger.info(f"Valid {event.signal_type} signal for {event.symbol} "
...
@conditional_event_handler('trading_signal', 
                         lambda e: e.signal_type != 'hold' and e.strength > 0.7)
def execute_strong_signals(event: TradingSignalEvent):
    ...
    try:
        ...
        logger.info(f"Created {event.signal_type} order for {position_size} "
...
def update_strategy_metrics(event: TradingSignalEvent):
    ...
    try:
        ...
        strategy_metrics['avg_strength'] = (
            (current_avg * (total - 1) + event.strength) / total
        )
...
@conditional_event_handler('trading_signal', 
                         lambda e: e.signal_type in ['buy', 'sell'])
