...
class BinanceFeeCalculator:
    ...
    def calculate_trading_fee(self, price: float, quantity: float, is_maker: bool = False, is_futures: bool = True) -> float:
        """
        ...
        self.logger.debug(f"Trading fee: ${fee:.6f} ({fee_rate*100:.3f}% of ${trade_value:.2f})")
        return fee
    
    def calculate_funding_fee(self, position_value: float, hours_held: float, funding_rate: float = None) -> float:
        """
        ...
        self.logger.debug(f"Funding fee: ${total_funding_fee:.6f} ({funding_periods:.2f} periods x {funding_rate*100:.4f}%)")
        return total_funding_fee
    
    def calculate_borrowing_cost(self, borrowed_amount: float, hours_held: float) -> float:
        """
        ...
        self.logger.debug(f"Borrowing cost: ${borrowing_cost:.6f} (${borrowed_amount:.2f} for {hours_held:.1f}h)")
    ...
    def calculate_total_position_cost(
        ...
        entry_fee = self.calculate_trading_fee(entry_price, quantity, is_maker=False, is_futures=True)
        exit_fee = self.calculate_trading_fee(exit_price, quantity, is_maker=False, is_futures=True)
        funding_fee = self.calculate_funding_fee(position_value, hours_held, funding_rate)
...
if __name__ == "__main__":
    ...
    print(f"Fee impact: ${calculation['gross_pnl'] - calculation['net_pnl']:.6f} ({(calculation['total_fees']/abs(calculation['gross_pnl'])*100):.2f}% of gross P&L)")