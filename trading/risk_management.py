...
class RiskManager:
        ...
        try:
            ...
            if market_data is not None and len(market_data) > 14:
                atr = ta.volatility.AverageTrueRange(
                    market_data['high'], market_data['low'], market_data['close'], window=14
                ...
                self.logger.info(f"📊 Volatility: {volatility_pct:.2f}%, Factor: {self.volatility_factor:.2f}, Appetite: {self.risk_appetite}")
            ...
            if len(self.recent_trades) >= 5:
                ...
                self.win_rate = len(winning_trades) / len(recent_pnl) if recent_pnl else 0.5
                ...
                self.logger.info(f"🎯 Win Rate: {self.win_rate:.1%}, Performance Factor: {self.performance_factor:.2f}")
    ...
    def enforce_minimum_leverage(self, current_leverage: float) -> float:
        ...
        if current_leverage < self.leverage_target:
            self.logger.warning(f"⚡ Leverage {current_leverage}x below target. Enforcing {self.leverage_target}x")
            return self.leverage_target
        return min(current_leverage, 125.0)  # Cap at 125x for safety
    
    def calculate_dynamic_position_size(self, available_capital: float, current_price: float, 
                                      signal_confidence: float, leverage: float = 50.0) -> float:
        """
        ...
        volatility_multiplier = (2 - self.volatility_factor) if self.volatility_factor < 1.5 else 0.7
        
        # Adjust for performance
        performance_multiplier = 1.2 if self.win_rate > 0.6 else 0.8 if self.win_rate < 0.4 else 1.0
        
        # Calculate final position size
        risk_amount = available_capital * base_risk * confidence_multiplier * volatility_multiplier * performance_multiplier
        position_value = risk_amount * effective_leverage
        position_size = position_value / current_price
        
        # Ensure reasonable bounds
        min_size = 0.0001  # Minimum BTC
        max_size = (available_capital * 0.8 * effective_leverage) / current_price  # 80% of capital max
        
        final_size = max(min_size, min(position_size, max_size))
        
        self.logger.info(f"💰 Dynamic sizing: {final_size:.6f} BTC (Lev: {effective_leverage}x, Conf: {signal_confidence:.1%}, Risk: {base_risk:.1%})")
        return final_size
    ...
    def manage_positions(self, data: Dict[str, pd.DataFrame] = None):
        ...
        for symbol, position in list(self.open_positions.items()):
            ...
            if pnl_pct < -(self.dynamic_stop_loss_pct * 100):
                self.logger.warning(f"🛑 DYNAMIC STOP LOSS: {symbol} at {pnl_pct:.2f}% (threshold: {self.dynamic_stop_loss_pct:.1%})")
                self._execute_emergency_close(symbol, position, current_price, 'STOP_LOSS')
                continue
            
            # DYNAMIC TAKE PROFIT - Based on current parameters
            if pnl_pct > (self.dynamic_take_profit_pct * 100):
                self.logger.info(f"💰 DYNAMIC TAKE PROFIT: {symbol} at {pnl_pct:.2f}% (threshold: {self.dynamic_take_profit_pct:.1%})")
                self._execute_emergency_close(symbol, position, current_price, 'TAKE_PROFIT')
                continue
    ...
    def _execute_emergency_close(self, symbol: str, position: Dict[str, Any], current_price: float, reason: str):
        """
        ...
        try:
            ...
            success = self.executor.place_order(symbol, close_side.lower(), quantity, current_price)
            if success:
                pnl = position.get('pnl', 0)
                self.logger.info(f"✅ {reason}: Closed {side} position of {quantity:.6f} {symbol} with PnL: ${pnl:.2f}")
    ...
    def _manage_dynamic_trailing_stop(self, symbol: str, position: Dict[str, Any], current_price: float):
        """
        ...
        if side.upper() == 'BUY':
            ...
            if current_price > trailing_info['highest_price']:
                ...
                if not trailing_info['activated'] and current_price > position.get('entry_price', 0) * 1.01:
                    trailing_info['activated'] = True
                    self.logger.info(f"🎯 Trailing stop activated for {symbol} at 1% profit")
                
                # Update stop price if activated
                if trailing_info['activated']:
                    new_stop_price = current_price * (1 - trailing_info['trail_distance_pct'])
                    if new_stop_price > trailing_info['stop_price']:
                        trailing_info['stop_price'] = new_stop_price
                        self.logger.debug(f"📈 Updated trailing stop for {symbol}: ${new_stop_price:.2f}")
            
            # Check if stop hit
            if trailing_info['activated'] and current_price <= trailing_info['stop_price']:
                self.logger.info(f"🛑 Dynamic trailing stop triggered for {symbol} at ${current_price:.2f}")
                self._execute_emergency_close(symbol, position, current_price, 'TRAILING_STOP')
        
        else:  # SHORT position
            if current_price < trailing_info['highest_price']:
                trailing_info['highest_price'] = current_price
                
                if not trailing_info['activated'] and current_price < position.get('entry_price', 0) * 0.99:
                    trailing_info['activated'] = True
                    self.logger.info(f"🎯 Trailing stop activated for SHORT {symbol} at 1% profit")
                
                if trailing_info['activated']:
                    new_stop_price = current_price * (1 + trailing_info['trail_distance_pct'])
                    if new_stop_price < trailing_info['stop_price']:
                        trailing_info['stop_price'] = new_stop_price
                        self.logger.debug(f"📉 Updated trailing stop for SHORT {symbol}: ${new_stop_price:.2f}")
            
            if trailing_info['activated'] and current_price >= trailing_info['stop_price']:
                self.logger.info(f"🛑 Dynamic trailing stop triggered for SHORT {symbol} at ${current_price:.2f}")
                self._execute_emergency_close(symbol, position, current_price, 'TRAILING_STOP')
    ...
    def calculate_take_profit(self, entry_price: float, volatility: float) -> float:
        ...
        """
        return max(entry_price + (3 * volatility), entry_price * (1 + self.dynamic_take_profit_pct))
