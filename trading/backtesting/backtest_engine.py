...
class BacktestEngine:
    ...
    def _process_signal(self, signal: Dict[str, Any], row: pd.Series):
        ...
        if position_size > 0:
            self._open_trade(side, row['close'], position_size, row['timestamp'], signal)
    ...
    def _open_trade(self, side: str, price: float, quantity: float, 
    ...
    def _close_trade(self, trade: Trade, exit_price: float, timestamp: datetime):
        ...
        logger.debug(f"Closed {trade.side} trade at {exit_price} with PnL: {trade.pnl:.2f}")
    ...
    def _calculate_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "total_return_pct": ((self.equity - self.config.initial_balance) / 
                                self.config.initial_balance * 100)
        }
        
        # Calculate Sharpe ratio
        if len(self.equity_curve) > 1:
            returns = pd.Series([self.equity_curve[i][1] / self.equity_curve[i-1][1] - 1 
    ...
    def plot_results(self, save_path: Optional[str] = None):
        ...
        try:
            ...
            returns = [t.pnl_percentage for t in self.trades if t.status in ['CLOSED', 'STOPPED_OUT']]
            if returns:
