...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--strategy", type=str, required=True, help="Path to strategy module"
    )

    parser.add_argument(
        "--data", type=str, required=True, help="Path to historical data file"
    )
...
def display_results(results: dict, console: Console):
    ...
    if "walk_forward" in results:
        ...
        for i, (train, test, params) in enumerate(
            zip(
        ...
        with Progress() as progress:
            if args.mode in ["monte_carlo", "all"]:
                task = progress.add_task(
                    "[cyan]Running Monte Carlo simulations...", total=1
                )
            ...
            if args.mode in ["walk_forward", "all"]:
                task = progress.add_task(
                    "[cyan]Running walk-forward analysis...", total=1
                )
