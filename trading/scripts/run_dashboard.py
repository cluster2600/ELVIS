...
class TradingDashboard:
    ...
    def __init__(self, logger=None):
        ...
        self.logger.info(
            f"Running in {'Testnet' if self.is_testnet else 'Production'} mode"
        )
            ...
            try:
                while True:
                    ...
                    if not self.dashboard_manager.is_running():
                        self.logger.info(
                            "Dashboard UI stopped, continuing trading logic"
                        )
    ...
    def _update_real_data(self) -> None:
        ...
        try:
            ...
            trades = self.executor.client.get_account_trades(
        ...
        if current_price <= 0:
            ...
            self.logger.info(
                f"Updated dashboard - Mode: {'Production' if self.config['PRODUCTION_MODE'] else 'Testnet'}, Portfolio: ${total_balance:.2f}, Price: ${current_price:.2f}"
            )
        self.logger.info(f"Open positions: {len(open_positions)}")
        for pos in open_positions:
            self.logger.info(
                f"Position: {pos['symbol']} - Size: {pos['size']}, PnL: ${pos['pnl']:.2f} ({pos['pnl_percentage']:.2f}%)"
            )
    ...
    def _generate_signals(self, data: pd.DataFrame) -> tuple:
        ...
        try:
            df = data.copy()
            df["ema_short"] = talib.EMA(
                df["close"].values, timeperiod=self.ema_short_period
            )
            df["ema_long"] = talib.EMA(
                df["close"].values, timeperiod=self.ema_long_period
            )
            ...
            buy_signal = (
                ...
                and (
                    pd.isna(latest["volatility"])
                    or latest["volatility"] > self.volatility_threshold
                )
            )
            sell_signal = (
                latest["ema_short"] < latest["ema_long"]
                or latest["rsi"] > self.rsi_overbought
            )

            # Enhanced logging for signal generation
            self.logger.info(
                f"Signal Check: EMA9={latest['ema_short']:.2f}, EMA21={latest['ema_long']:.2f}, RSI={latest['rsi']:.2f}, Volatility={latest['volatility']:.4f if not pd.isna(latest['volatility']) else 'NaN'}, Buy={buy_signal}, Sell={sell_signal}"
            )
            if df.empty:
    ...
    def _execute_trading_strategy(self):
        ...
        if len(price_data) < 2:
            self.logger.debug(
                f"Insufficient price data for strategy: {len(price_data)} prices"
            )
            return
        ...
        position = next(
            (p for p in self.config["open_positions"] if p["symbol"] == "BTCUSDT"), None
        )
        ...
        if signal == "BUY" and not position:
            ...
            try:
                order = self.executor.execute_sell(
                    "BTCUSDT", abs(position["size"]), current_price
                )
                self.config["total_trades"] += 1
                if position["pnl"] > 0:
            ...
            self.logger.debug(
                f"No trade action - Signal: {signal}, Position exists: {bool(position)}"
            )
    ...
    def _update_positions(self):
        ...
        try:
            ...
            self.logger.info(
                f"Positions updated: {len(self.config['open_positions'])} open"
            )
        except Exception as e:
