        ...
        try:
            if self.is_testnet:  # Paper trading mode - use PostgreSQL
                ...
                for pos in positions:
                    ...
                    pnl = (
                        (current_price - entry_price) * size if current_price > 0 else 0
                    )
                    pnl_percentage = (
        ...
        if signal == "BUY" and not position:
            ...
            try:
                order = self.executor.execute_buy(
                    "BTCUSDT", self.trade_size, current_price
                )
                self.config["total_trades"] += 1
                order_msg = (
                    f"Buy order executed: {self.trade_size} BTC at ${current_price:.2f}"
                )
