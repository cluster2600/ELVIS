...
from trading.backtesting import BacktestConfig, BacktestEngine
from trading.strategies.base_strategy import BaseStrategy
from trading.strategies.ensemble_strategy import EnsembleStrategy
from utils.logger_config import setup_logging

# Setup logging
logger = setup_logging(app_name="BACKTEST", log_level="INFO", enable_file_logging=True)
...
def main():
    ...
    parser.add_argument(
        "--timeframe", type=str, default="5min", help="Timeframe for data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-trades", type=int, default=3, help="Maximum concurrent trades"
    )
    parser.add_argument(
        "--risk-per-trade", type=float, default=0.02, help="Risk per trade (0.02 = 2%)"
    )
    ...
    logger.info(
        f"Loaded {len(data)} data points from {data['timestamp'].min()} to {data['timestamp'].max()}"
    )
    ...
    if args.strategy == "sma":
        ...
        logger.warning(
            "Ensemble strategy requires trained models. Using simple strategy instead."
        )
    ...
    logger.info(
        f"Total Return: ${stats['total_return']:.2f} ({stats['total_return_pct']:.2%})"
    )
    logger.info("=" * 50)
