...
from utils.logger_config import TradingLogger, setup_logging
...
class ApplicationBootstrapper:
    ...
    def _setup_logging(self) -> None:
        ...
        self.logger = setup_logging(
            ...
            enable_json_logging=os.getenv("ENABLE_JSON_LOGS", "false").lower()
            == "true",
    ...
    def _register_configurations(self) -> None:
        ...
        container.register_singleton(
            "api_config", lambda: API_CONFIG
        )  # Register as singleton since it's an object
        container.register_configuration(
            "app_config",
            {
                "mode": self.mode,
                "log_level": self.log_level,
    ...
    def _register_core_services(self) -> None:
        ...
        # Async Task Manager
        container.register_singleton(
            "async_task_manager", lambda: AsyncTaskManager(max_concurrent=10)
        )
        ...
        try:
            ...
            container.register_singleton(
                "performance_monitor", create_mock_performance_monitor
            )
        ...
        def create_trade_analyzer():
            ...
            try:
                ...
                logger.info(
                    f"TradeAnalyzer initialized with {len(live_trades)} live trades from database"
                )
        ...
        container.register_singleton(
            "system_monitor", lambda: SystemMonitor(api_url="https://api.binance.com")
        )
        ...
        container.register_singleton(
            "market_regime_detector", lambda: MarketRegimeDetector()
        )
        ...
        def create_strategy_manager():
            ...
            if "mean_reversion_strategy" in container._providers:
                try:
                    strategies["mean-reverting"] = container.get(
    ...
    def _register_data_services(self) -> None:
        ...
        try:
            ...
            def create_price_fetcher():
                ...
                primary_symbols = SYMBOLS_CONFIG.get(
                    "PRIMARY_SYMBOLS", ["BTCUSDT", "BNBUSDT"]
                )
                logger.info(
                    f"🔄 PriceFetcher initialized with symbols: {primary_symbols}"
                )
    ...
    def _register_trading_services(self) -> None:
        ...
        def create_exchange_manager():
            ...
            kraken_api_key = getattr(
                api_config, "KRAKEN_API_KEY", None
            ) or api_config.get("KRAKEN_API_KEY")
            kraken_secret = getattr(
                api_config, "KRAKEN_API_SECRET", None
            ) or api_config.get("KRAKEN_API_SECRET")
            ...
            coinbase_api_key = getattr(
                api_config, "COINBASE_API_KEY", None
            ) or api_config.get("COINBASE_API_KEY")
            coinbase_secret = getattr(
                api_config, "COINBASE_API_SECRET", None
            ) or api_config.get("COINBASE_API_SECRET")
            coinbase_passphrase = getattr(
                api_config, "COINBASE_PASSPHRASE", None
            ) or api_config.get("COINBASE_PASSPHRASE")
            ...
            if coinbase_api_key and coinbase_secret and coinbase_passphrase:
                ...
                exchange_manager.add_exchange(
                    "coinbase", CoinbaseExecutor, coinbase_config
                )
        ...
        container.register_singleton(
            "advanced_order_manager", create_advanced_order_manager
        )
        ...
        def create_executor():
            ...
            use_futures = (
                app_config["mode"] == "futures_testnet"
                or trading_config.get("DEFAULT_MODE") == "futures_testnet"
            )
            is_testnet = app_config["mode"] in ["paper", "futures_testnet"]

            logger.info(
                f"Creating executor with mode: {app_config['mode']}, use_futures: {use_futures}, is_testnet: {is_testnet}"
            )
            ...
            if not success:
                logger.warning(
                    "Failed to initialize executor with futures, falling back to paper trading"
                )
        ...
        def create_strategy():
            ...
            if strategy_mode == "research":
                ...
                social_data_enabled = (
                    os.getenv("SOCIAL_DATA_ENABLED", "true").lower() == "true"
                )
                rolling_training = (
                    os.getenv("ROLLING_TRAINING_ENABLED", "true").lower() == "true"
                )
                ...
                logger.info(
                    "⚠️ Emergency mode: Reduced trading frequency and higher profit targets"
                )
                ...
                logger.info(
                    f"🎯 Ensemble Strategy initialized with symbols: {primary_symbols}"
                )
        ...
        try:
            ...
            container.register_factory(
                "grid_strategy", lambda: GridStrategy(logger=container.get("logger"))
            )
        except ImportError as e:
        ...
        try:
            from trading.strategies.trend_following_strategy import (
    ...
    def _register_models(self) -> None:
        ...
        try:
            ...
            container.register_factory(
                "random_forest_model", lambda: RandomForestModel()
            )
        except ImportError as e:
        ...
        try:
            ...
            container.register_factory(
                "neural_network_model", lambda: NeuralNetworkModel(sequence_length=60)
            )
        except ImportError as e:
        ...
        try:
            ...
            container.register_factory(
                ...
                lambda: TransformerModel(
                    input_dim=10, d_model=128, nhead=8, num_layers=4
                ),
            )
        except ImportError as e:
        ...
        try:
            ...
            for module in [
                ...
                try:
                    ...
                    self.logger.info(
                        f"Registered event handlers from {module.__name__}"
                    )
                except ImportError:
...
def bootstrap_application(
    mode: str = "paper", log_level: str = "INFO"
