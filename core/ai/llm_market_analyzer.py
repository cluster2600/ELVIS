...
class LLMMarketAnalyzer:
    ...
    def __init__(self, config: LLMConfig, fallback_configs: Optional[List[LLMConfig]] = None):
        self.config = config
    ...
    def _initialize_providers(self):
        ...
        # OpenAI
        if self.config.provider == "openai" or any(c.provider == "openai" for c in self.fallback_configs):
        ...
        # Anthropic Claude
        if ANTHROPIC_AVAILABLE and (self.config.provider == "anthropic" or any(c.provider == "anthropic" for c in self.fallback_configs)):
            if self.config.api_key:
                providers["anthropic"] = anthropic.Client(api_key=self.config.api_key)
                logger.info("✅ Anthropic provider initialized")
        
        # Google Gemini
        if GEMINI_AVAILABLE and (self.config.provider == "gemini" or any(c.provider == "gemini" for c in self.fallback_configs)):
            if self.config.api_key:
        ...
        self.current_provider = self.config.provider if self.config.provider in providers else None
        
        if not self.current_provider:
            logger.warning("⚠️  No LLM providers available, analysis will use fallback logic")
    
    async def analyze_market_conditions(self, 
                                      market_data: Dict[str, Any], 
                                      news_data: Optional[List[Dict]] = None,
                                      social_data: Optional[Dict] = None,
                                      technical_analysis: Optional[Dict] = None) -> MarketAnalysis:
        """
        ...
        try:
            ...
            context = self._prepare_analysis_context(market_data, news_data, social_data, technical_analysis)
            ...
            logger.info(f"📊 Market analysis complete: {analysis.sentiment.value} ({analysis.confidence.value})")
            return analysis
    ...
    def _prepare_analysis_context(self, market_data: Dict, news_data: Optional[List], 
                                social_data: Optional[Dict], technical_analysis: Optional[Dict]) -> Dict:
...
"""
        ...
        try:
            ...
            for fallback_config in self.fallback_configs:
                try:
                    ...
                    logger.info(f"✅ Fallback LLM succeeded: {fallback_config.provider}")
                    return result
                    
                except Exception as fallback_error:
                    logger.warning(f"⚠️  Fallback {fallback_config.provider} failed: {fallback_error}")
                    continue
                finally:
    ...
    def _parse_llm_response(self, response: str, market_data: Dict) -> MarketAnalysis:
        ...
        try:
            ...
            analysis = MarketAnalysis(
                ...
                price_prediction=parsed.get("price_prediction", {"direction": 0, "magnitude": 0.5}),
                risk_assessment=parsed.get("risk_assessment", {"risk_level": "medium", "volatility_warning": False}),
                trading_recommendation=parsed.get("trading_recommendation", "Hold current position"),
    ...
    def _create_fallback_analysis(self, market_data: Dict, llm_response: str = None) -> MarketAnalysis:
        ...
        return MarketAnalysis(
            ...
            price_prediction={"direction": direction, "magnitude": min(abs(price_change) / 20, 1.0)},
            risk_assessment={"risk_level": "medium", "volatility_warning": volatility > 20},
    ...
    def _analyze_news_sentiment(self, news_data: List[Dict]) -> float:
        ...
        positive_keywords = ["bullish", "surge", "rally", "breakthrough", "adoption", "positive"]
    ...
    def _extract_news_topics(self, news_data: List[Dict]) -> List[str]:
        ...
        topics = set()
        keywords = ["bitcoin", "ethereum", "regulation", "adoption", "institutional", "technical", "trading"]
    ...
    def _update_metrics(self, analysis: MarketAnalysis, response_time: float):
        ...
        self.performance_metrics["average_confidence"] = ((avg * (total - 1)) + current_confidence) / total
    ...
    def get_analysis_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for analysis in recent_analyses:
            sentiment_counts[analysis.sentiment.value] = sentiment_counts.get(analysis.sentiment.value, 0) + 1
            confidence_counts[analysis.confidence.value] = confidence_counts.get(analysis.confidence.value, 0) + 1
...
async def test_llm_analyzer():
    ...
    configs = [
        LLMConfig(provider="local", model="llama-3.1-8b", base_url="http://localhost:1234/v1", temperature=0.3),
        LLMConfig(provider="openai", model="gpt-4", api_key="your-openai-key", temperature=0.3),
    ]
    ...
    # Sample news data
    news_data = [
        {"title": "Bitcoin Surges as Institutional Adoption Increases", "content": "Major companies announce Bitcoin treasury adoption..."},
        {"title": "Crypto Market Shows Strong Bullish Momentum", "content": "Technical indicators suggest continued upward trend..."}
    ]
    ...
    try:
        ...
        print(f"   Success Rate: {summary['performance_metrics']['successful_analyses']}/{summary['performance_metrics']['total_analyses']}")
        print(f"   Average Confidence: {summary['performance_metrics']['average_confidence']:.2f}")
