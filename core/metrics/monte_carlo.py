...
class MonteCarloSimulator:
    ...
    def _simulate_returns(
        ...
        simulated_returns[~wins] = np.random.normal(
            avg_loss, abs(avg_loss) / 2, np.sum(~wins)
        )
    ...
    def _calculate_metrics(
        self, equity_curve: np.ndarray, returns: np.ndarray
    ) -> Dict[str, float]:
        """
    ...
    def _run_single_simulation(
        ...
        simulated_returns = self._simulate_returns(
            returns, win_rate, win_loss_ratio, num_trades
        )
    ...
    def run_simulation(
        ...
        try:
            self.logger.info(
                f"Running Monte Carlo simulation with {self.num_simulations} iterations"
            )
            ...
            if win_loss_ratio is None:
                avg_win = np.mean(returns[returns > 0]) if np.any(returns > 0) else 0.01
                avg_loss = (
                    abs(np.mean(returns[returns < 0])) if np.any(returns < 0) else 0.01
                )
            ...
            self.logger.info(
                f"Simulation parameters: win_rate={win_rate:.2f}, win_loss_ratio={win_loss_ratio:.2f}, num_trades={num_trades}"
            )
            ...
            if self.parallel and self.num_processes > 1:
                # Parallel simulation
                self.logger.info(
                    f"Running simulations in parallel with {self.num_processes} processes"
                )
                ...
                with mp.Pool(processes=self.num_processes) as pool:
                    simulation_results = list(
                        tqdm(
                            pool.imap(
                                lambda _: sim_func(), range(self.num_simulations)
                            ),
    ...
    def _aggregate_results(
        self, simulation_results: List[Dict[str, float]]
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_plots(self, simulation_results: List[Dict[str, float]]) -> None:
        ...
        plt.figure(figsize=(10, 6))
        sns.histplot(
            results_df["profit_factor"].clip(upper=5), kde=True
    ...
    def _save_results(self, strategy_name: str, results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            results_file = os.path.join(
                results_dir, f"{strategy_name}_monte_carlo.json"
            )
