...
def conditional_event_handler(event_types: Union[str, List[str]], 
...
def throttled_event_handler(event_types: Union[str, List[str]], 
...
def buffered_event_handler(event_types: Union[str, List[str]], 
                         buffer_size: int = 10,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def buffered_wrapper(event, *args, **kwargs):
            ...
            with buffer_lock:
                ...
                should_flush = (
                    len(buffers[event_type]) >= buffer_size or
                    current_time - last_flush[event_type] >= flush_interval_ms
                )
