...
class TradingFeaturePipeline:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(df) < 50:
            self.logger.warning(f"Insufficient data for feature generation: {len(df)} rows")
        ...
        try:
            ...
            self.logger.info(f"✅ Generated {len([c for c in features_df.columns if c not in ['open', 'high', 'low', 'close', 'volume', 'timestamp']])} features")
    ...
    def _add_price_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.feature_groups['price'] = [col for col in df.columns if 
                                       any(x in col for x in ['price', 'sma', 'ema', 'body', 'shadow', 'hammer', 'doji', 'gap', 'position'])]
    ...
    def _add_volume_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.feature_groups['volume'] = [col for col in df.columns if 
                                        any(x in col for x in ['volume', 'obv', 'vwap'])]
    ...
    def _add_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [14, 28]:
            ...
            df[f'normalized_volatility_{window}'] = df[f'volatility_{window}'] / df['close']
        ...
        self.feature_groups['volatility'] = [col for col in df.columns if 
                                           any(x in col for x in ['volatility', 'atr', 'true_range', 'gk_'])]
    ...
    def _add_momentum_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [14, 21]:
            ...
            rolling_high = df['high'].rolling(window).max()
            rolling_low = df['low'].rolling(window).min()
            df[f'williams_r_{window}'] = -100 * (rolling_high - df['close']) / (rolling_high - rolling_low)
            
            # Stochastic %K
            df[f'stoch_k_{window}'] = 100 * (df['close'] - rolling_low) / (rolling_high - rolling_low)
            df[f'stoch_d_{window}'] = df[f'stoch_k_{window}'].rolling(3).mean()
        
        self.feature_groups['momentum'] = [col for col in df.columns if 
                                         any(x in col for x in ['roc', 'momentum', 'acceleration', 'williams', 'stoch'])]
    ...
    def _add_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.feature_groups['technical'] = [col for col in df.columns if 
                                          any(x in col for x in ['rsi', 'macd', 'bb_', 'adx', 'di_', 'cci', 'mfi', 'psar'])]
    ...
    def _add_market_structure_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['regime_volatility'] = (df['volatility_14'] > df['volatility_14'].rolling(50).mean()).astype(int)
        df['regime_trend'] = (df['close'] > df['sma_50']).astype(int)
        
        self.feature_groups['market_structure'] = [col for col in df.columns if 
                                                 any(x in col for x in ['resistance', 'support', 'trend', 'local_', 'regime'])]
    ...
    def _add_time_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.feature_groups['time'] = [col for col in df.columns if 
                                     any(x in col for x in ['hour', 'day', 'month', 'session', 'weekend'])]
    ...
    def _add_feature_interactions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['momentum_volatility_ratio'] = df['momentum_10'] / (df['volatility_14'] + 1e-8)
            
            # Trend confirmation signals
            df['trend_confirmation'] = ((df['price_to_sma_20'] > 1) & 
                                       (df['rsi_14'] > 50) & 
    ...
    def _generate_minimal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            features_df['volume_change'] = features_df['volume'].pct_change() if 'volume' in features_df.columns else 0
            
            # Simple moving averages
            features_df['sma_5'] = features_df['close'].rolling(5).mean()
            features_df['sma_20'] = features_df['close'].rolling(20).mean()
            features_df['price_to_sma_20'] = features_df['close'] / features_df['sma_20']
    ...
    def validate_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        validation = {
            ...
            'infinite_counts': np.isinf(df.select_dtypes(include=[np.number])).sum().to_dict(),
            'zero_variance_features': [col for col in numeric_cols if df[col].std() == 0],
            'feature_ranges': {col: {'min': df[col].min(), 'max': df[col].max()} 
                             for col in numeric_cols[:10]},  # First 10 features only
