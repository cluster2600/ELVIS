...
class EnsembleModel(BaseModel):
    ...
    def __init__(self, logger: logging.Logger, **kwargs):
        ...
        self.threshold: float = kwargs.get(
            "threshold", 0.5
        )  # Used for 'hard' voting or final decision
        ...
        if self.weights and len(self.weights) != len(self.models):
            self.logger.warning(
                f"Number of weights ({len(self.weights)}) does not match number of models ({len(self.models)}). Ignoring weights."
            )
            self.weights = None
        if not self.weights:
            if self.models:
    ...
    def _load_models_from_configs(self):
        ...
        self.models = []
        self.logger.info(
            f"Loading {len(self.model_configs)} sub-models from configuration..."
        )
        for config in self.model_configs:
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to load sub-model '{name}' from config {config}: {e}",
                    exc_info=True,
                )
    ...
    def _populate_configs_from_models(self):
        ...
        for name, model_instance in self.models:
            ...
            if model_path:
                ...
                self.logger.warning(
                    f"Could not determine path for model '{name}' ({model_instance.__class__.__name__}). It won't be saved in ensemble config."
                )
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series) -> None:
        ...
        """
        self.logger.info(
            "EnsembleModel 'train' called. Ensuring sub-models are loaded."
        )
        ...
        if not self.models:
            self.logger.error(
                "No sub-models configured or loaded for the ensemble. Cannot 'train'."
            )
            return
        ...
        try:
            if not X_train.empty:
                ...
                self.logger.warning(
                    "No training data provided to validate sub-model prediction readiness."
                )
        except Exception as e:
            self.logger.error(
                f"Error validating sub-model prediction during ensemble 'train': {e}",
                exc_info=True,
            )
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        for i, (name, model) in enumerate(self.models):
            try:
                ...
                self.logger.error(
                    f"Error predicting with sub-model '{name}': {e}", exc_info=True
                )
        ...
        try:
            ...
            normalized_weights = (
                [w / total_weight for w in effective_weights]
                if total_weight > 0
                else [1.0 / valid_models_count] * valid_models_count
            )
            ...
            if self.voting == "soft":
                ...
                weighted_preds = [
                    pred * weight for pred, weight in zip(np_preds, normalized_weights)
                ]
                ...
                self.logger.info(
                    f"Aggregated {valid_models_count} predictions using soft voting."
                )

            elif self.voting == "hard":
                # Majority vote (assuming predict returns class labels 0 or 1)
                np_preds = np.array([np.round(p) for p in all_predictions]).astype(
                    int
                )  # Ensure binary predictions
                # Weighted majority vote
                weighted_votes = np.tensordot(
                    normalized_weights, np_preds, axes=([0], [0])
                )
                final_predictions = (weighted_votes >= self.threshold).astype(
                    int
                )  # Use threshold for final decision
                self.logger.info(
                    f"Aggregated {valid_models_count} predictions using hard voting."
                )
    ...
    def evaluate(self, X_test: pd.DataFrame, y_test: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            predictions = self.predict(
                X_test
            )  # Returns probabilities or classes based on voting
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        try:
            ...
            for name, model in self.models:
                if hasattr(model, "get_feature_importance"):
                    try:
                        ...
                        self.logger.error(
                            f"Error getting feature importance from {name}: {e}"
                        )

            if not all_importances:
                self.logger.warning(
                    "No feature importance available from any sub-model."
                )
                return pd.DataFrame()
            ...
            combined["importance"] = pd.to_numeric(
                combined["importance"], errors="coerce"
            )
            combined = combined.dropna(subset=["importance"])

            avg_importance = (
                combined.groupby("feature")["importance"].mean().reset_index()
            )
    ...
    def save(self, path: str) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error saving Ensemble configuration to {save_path}: {e}",
                exc_info=True,
            )
    ...
    def load(cls, path: str) -> "EnsembleModel":
        ...
        try:
            ...
            logger.info(
                f"Ensemble configuration loaded. {len(instance.models)} sub-models loaded."
            )
            return instance

        except Exception as e:
            logger.error(
                f"Error loading Ensemble configuration from {path}: {e}", exc_info=True
            )
    ...
    def set_params(self, **params) -> None:
        ...
        if "weights" in params:
            ...
            if new_weights and len(new_weights) == len(self.models):
                ...
                self.logger.warning(
                    f"Attempted to set weights with incorrect length ({len(new_weights)}). Expected {len(self.models)}. Keeping existing weights."
                )
