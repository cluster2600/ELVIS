...
class NeuralNetworkModel(BaseModel):
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            scaler_path = os.path.join(
                os.path.dirname(self.model_path), "nn_scaler.pkl"
            )
    ...
    def _build_model(self) -> tf.keras.Model:
        ...
        model.add(
            LSTM(
                self.lstm_units[0], return_sequences=True, input_shape=self.input_shape
            )
        )
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            if len(X) < sequence_length:
                self.logger.warning(
                    f"Not enough data for prediction. Need at least {sequence_length} samples."
                )
                return np.zeros(len(X))
            ...
            self.logger.error(
                f"Error making predictions with Neural Network model: {e}"
            )
            return np.zeros(len(X))
    ...
    def evaluate(self, X_test: pd.DataFrame, y_test: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            # Extract metrics
            metrics = {"loss": evaluation[0], "accuracy": evaluation[1]}
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        """
        self.logger.warning(
            "Feature importance not directly available for Neural Network models"
        )
        return pd.DataFrame()
