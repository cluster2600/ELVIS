...
class EnhancedRFIntegration:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            if os.path.exists(os.path.join(self.model_path, "rf_model.pkl")):
                self.model = EnhancedRandomForestModel.load(
                    self.model_path, self.logger
                )
    ...
    def get_prediction(
        self, market_data: Dict[str, Any], trading_features: Dict[str, Any]
        ...
        try:
            ...
            explanation = (
                self.model.explain_prediction(latest_features)
                if self.model.enable_shap
                else None
            )
            ...
            metadata = {
                ...
                "top_features": (
                    feature_importance.head(5).to_dict("records")
            ...
            if explanation is not None:
                metadata["shap_explanation"] = {
                    "values": (
                        explanation.tolist() if hasattr(explanation, "tolist") else []
                    ),
            ...
            if (
                self.auto_retrain
                and datetime.now() - self.last_retrain_check
                > self.retrain_check_interval
            ):
            ...
            self.logger.debug(
                f"Enhanced RF prediction: {ensemble_prediction} (confidence: {confidence:.3f})"
            )
            ...
            for col in required_cols:
                if col not in df.columns:
                    if col == "volume":
                        ...
                        df[col] = market_data.get(
                            "price", market_data.get("close", 50000.0)
                        )
    ...
    def _calculate_ensemble_weights(
        self, confidence: float, probabilities: np.ndarray
    ) -> Dict[str, float]:
    ...
    def _check_retrain_trigger(self):
        ...
        try:
            ...
            if recent_confidence < 0.5:
                self.logger.warning(
                    "⚠️ Model confidence degraded, retraining recommended"
                )
    ...
    def train_model(
        ...
        self.logger.info(
            f"🚀 Training enhanced RF model with {len(training_data)} samples"
        )

        try:
            # Generate features if raw OHLCV data is provided
            if (
                "close" in training_data.columns
                and len(self.feature_pipeline.get_feature_names()) == 0
            ):
            ...
            self.logger.info(
                f"📊 Full dataset accuracy: {full_metrics.get('accuracy', 0):.3f}"
            )
    ...
    def export_model_insights(
        self, output_path: str = "model_insights"
    ) -> Dict[str, str]:
