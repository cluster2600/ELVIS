...
import json
import os
from utils.paper_trade_db import get_conn, get_all_trades
...
class TradingFeatureExtractor:
    ...
    def extract_features_from_trades(self, trades: List[Tuple], market_data: Dict = None) -> np.ndarray:
        """
        ...
        try:
            ...
            if trades and len(trades) > 0:
                ...
                recent_pnl = [float(trade[6]) for trade in recent_trades if trade[6] is not None]
                recent_fees = [float(trade[7]) for trade in recent_trades if trade[7] is not None]
...
class TradingRewardCalculator:
    ...
    def calculate_reward(self, action: int, current_price: float, last_price: float, 
                        position: float, trade_result: Dict = None) -> float:
        """
            ...
            if action == 0:  # HOLD
                ...
                if trade_result:
                    ...
                    if price_change < 0:
                        ...
                        return -price_change * 100 - 0.4  # Penalty for selling before rise
...
class DQNNetwork(nn.Module):
    ...
    def __init__(self, input_size: int = 16, hidden_size: int = 128, output_size: int = 3):
...
class TradingDQNAgent:
    ...
    def __init__(self, state_size: int = 16, action_size: int = 3, 
                 learning_rate: float = 0.001, logger: logging.Logger = None):
...
class TradingRLModel:
    ...
    def __init__(self, logger: logging.Logger, model_path: str = "models/trading_rl_model.pth"):
        self.logger = logger
    ...
    def train_on_historical_data(self, limit: int = 1000):
        ...
        try:
            ...
            if not trades:
                self.logger.info("No historical trades found, creating synthetic training data")
                ...
                try:
                    ...
                    if i + 1 < len(trades):
                        ...
                        reward = self.reward_calculator.calculate_reward(
                            action, next_trade[4], trade[4], 0, 
                            {'pnl': trade[6], 'fees': trade[7]}
                        )
                        
                        # Next state
                        next_state = self.feature_extractor.extract_features_from_trades(
                            trades[:i+2], {'price': next_trade[4], 'volume': next_trade[5] * 1000}
                        )
            ...
            self.logger.info(f"Training completed - Episode reward: {episode_reward:.2f}, Avg loss: {avg_loss:.6f}")
    ...
    def _create_synthetic_training_data(self) -> bool:
        ...
        try:
            self.logger.info("Creating synthetic training data for RL model initialization...")
            ...
            # Log training results
            avg_loss = sum(training_losses) / len(training_losses) if training_losses else 0
            self.logger.info(f"Synthetic training completed - Episode reward: {episode_reward:.2f}, Avg loss: {avg_loss:.6f}")
            self.logger.info(f"RL model initialized with synthetic data - ready for real trading")
    ...
    def predict_action(self, market_data: Dict, trades: List = None) -> Tuple[str, float]:
        """
        ...
        try:
            ...
            state = self.feature_extractor.extract_features_from_trades(trades, market_data)
        ...
        try:
            ...
            if len(self.agent.memory) > self.agent.batch_size:
                ...
                if self.agent.step_count % 10 == 0:  # Log every 10 steps
                    self.logger.info(f"Online learning - Reward: {reward:.3f}, Loss: {loss:.6f}")
