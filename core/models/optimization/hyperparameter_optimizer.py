...
class HyperparameterOptimizer:
    ...
    def optimize(
        ...
        def objective(trial):
            return self._objective_function(
                trial, X_train, y_train, X_val, y_val, cv_folds, scoring_metric
            )
    ...
    def _objective_function(
        ...
        try:
            ...
            return (
                -np.inf
                if scoring_metric in ["accuracy", "f1", "precision", "recall"]
                else np.inf
            )
    ...
    def _calculate_score(
        self, y_true: pd.Series, y_pred: np.ndarray, scoring_metric: str
    ) -> float:
    ...
    def _save_optimization_results(
        self, study: "optuna.Study", X_train: pd.DataFrame, y_train: pd.Series
    ) -> Dict[str, Any]:
        ...
        results_file = os.path.join(
            self.storage_path, f"{study.study_name}_results.json"
        )
        ...
        results["optimization_report"] = self._generate_optimization_report(
            study, trial_data
        )
    ...
    def _generate_optimization_report(
        self, study: "optuna.Study", trial_data: List[Dict]
    ) -> Dict[str, Any]:
        ...
        report = {
            ...
            "performance_stats": {
                ...
                "score_improvement": (
                    study.best_value - np.mean(values[:10]) if len(values) >= 10 else 0
                ),
            },
    ...
    def _generate_recommendations(
        self, study: "optuna.Study", values: List[float]
    ) -> Dict[str, str]:
        ...
        if len(values) > 0:
            if study.best_value > np.mean(values) + 2 * np.std(values):
                ...
                recommendations["performance"] = (
                    "Good optimization results, consider more trials"
                )
            else:
                recommendations["performance"] = (
                    "Limited improvement found, review feature engineering"
                )
        ...
        if best_params.get("n_estimators", 0) >= 400:
            recommendations["n_estimators"] = (
                "Consider reducing n_estimators for faster training"
            )
    ...
    def load_best_parameters(self, study_name: str = None) -> Optional[Dict[str, Any]]:
        ...
        if study_name is None:
            ...
            study_files = [
                f for f in os.listdir(self.storage_path) if f.endswith("_results.json")
            ]
            if not study_files:
                return None
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file in result_files:
                ...
                # Track best overall
                if (
                    summary["best_overall_score"] is None
                    or results.get("best_value", 0) > summary["best_overall_score"]
                ):
