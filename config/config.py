...
class APIConfig:
    ...
    def BINANCE_API_KEY(self):
        return self._secrets.get_secret("BINANCE_API_KEY", "api_keys") or os.getenv(
            "BINANCE_API_KEY", "your_binance_api_key_here"
        )

    @property
    def BINANCE_API_SECRET(self):
        return self._secrets.get_secret("BINANCE_API_SECRET", "api_keys") or os.getenv(
...
PAPER_TRADING_CONFIG = {
    "INITIAL_USDT_BALANCE": 100.0,  # Starting USDT balance ($100 paper deposit)
    "INITIAL_BNB_BALANCE": 0.0,  # No BNB seed - start from pure $100 USDT
...
SYMBOLS_CONFIG = {
    "PRIMARY_SYMBOLS": [
        "BTCUSDT",
        "BNBUSDT",
    ],  # Primary trading pairs - REMOVED BNBBTC due to pricing issues
    "SECONDARY_SYMBOLS": ["ETHUSDT", "ADAUSDT"],  # Secondary pairs (optional)
    "STABLE_PAIRS": ["BTCUSDT", "ETHUSDT"],  # Stable, high-liquidity pairs
    "FEE_OPTIMIZATION_PAIRS": [
        "BNBUSDT"
    ],  # Pairs for fee optimization - REMOVED BNBBTC
