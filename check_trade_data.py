...
def main():
    ...
    try:
        ...
        market_features = [
            ...
            if any(
                x in col.lower()
                for x in ["price", "volume", "momentum", "volatility", "sma"]
            )
        ]
        trading_features = [
            col
        ...
        time_features = [
            col
            for col in features_df.columns
            if any(x in col.lower() for x in ["hour", "day", "minute"])
        ]

        print(
            f"   🏪 Market features ({len(market_features)}): {', '.join(market_features[:5])}{'...' if len(market_features) > 5 else ''}"
        )
        print(
            f"   💰 Trading features ({len(trading_features)}): {', '.join(trading_features[:5])}{'...' if len(trading_features) > 5 else ''}"
        )
        ...
        if feature_info:
            ...
            print(
                f"   🔢 Numeric features: {feature_info.get('numeric_feature_count', 'N/A')}"
            )
            ...
            if means:
                print(
                    f"   💹 Avg PnL: {means.get('pnl', 'N/A'):.4f}"
                    if "pnl" in means
                    else ""
                )
                print(
                    f"   💸 Avg Fee: {means.get('fee', 'N/A'):.4f}"
                    if "fee" in means
                    else ""
                )
                print(
                    f"   🎯 Win Rate: {means.get('is_profitable', 'N/A'):.2%}"
                    if "is_profitable" in means
                    else ""
                )
