#!/usr/bin/env python3
"""
Train models using ALL individual paper trades from PostgreSQL without LLM features.
This approach maximizes training samples by using trade-level features.
"""
...
def train_on_all_trades(df, epochs=20, debug=False, args=None):
    """Train models on all individual trades without LLM features"""
    
    logger = setup_logger('trade_trainer')
    logger.info(f"🧠 Training models on {len(df)} individual trades...")
    
    # Prepare features and targets (no LLM features)
    exclude_columns = ['id', 'timestamp', 'symbol', 'side', 'target_up', 'target_profitable', 'target_return']
    exclude_columns.extend([col for col in df.columns if col.startswith('future_')])
    
    feature_columns = [col for col in df.columns if col not in exclude_columns]
    
    X = df[feature_columns].fillna(0)
    y_classification = df['target_up']
    y_regression = df['target_return'].fillna(0)
    ...
    if debug:
        ...
        for i, (feature, importance) in enumerate(feature_importance[:15]):
            logger.debug(f"    {i+1:2d}. 📊 {feature}: {importance:.4f}")
    ...
    model_files = {
        'classifier': f'models/all_trades_classifier_{timestamp}.joblib',
        'regressor': f'models/all_trades_regressor_{timestamp}.joblib',
        'scaler': f'models/all_trades_scaler_{timestamp}.joblib'
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train models on ALL individual paper trades (no LLM)')
    ...
    logger.info("🚀 Starting ALL TRADES Training Pipeline (No LLM)")
    logger.info("=" * 55)
    ...
    logger.info(f"   LLM Features: Disabled")
    ...
    try:
        ...
        # Step 4: Train models
        logger.info(f"\n🧠 Step 4: Training models on ALL trades...")
        ...
        print(f"\n🧠 ALL PAPER TRADES ELVIS Training (No LLM)")
        print("=" * 50)
        ...
        print(f"📊 Trained on ALL {metadata['training_config']['total_trades_loaded']:,} paper trades!")
