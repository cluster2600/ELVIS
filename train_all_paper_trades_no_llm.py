...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train models on ALL individual paper trades (no LLM)"
    )
    parser.add_argument(
        "--trades",
        type=int,
    ...
    try:
        ...
        classifier, regressor, scaler, metadata = train_on_all_trades(
            targets_df, epochs=args.epochs, debug=args.debug, args=args
        )
        ...
        print(
            f"📊 Trained on ALL {metadata['training_config']['total_trades_loaded']:,} paper trades!"
        )
