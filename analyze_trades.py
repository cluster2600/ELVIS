    ...
    try:
        ...
        if recent_trades:
            ...
            for trade in recent_trades[:10]:  # Show first 10
                print(f"  {trade[1]} | {trade[2]} | {trade[3]} | Price: ${trade[4]:.2f} | Qty: {trade[5]:.4f} | PnL: ${trade[6]:.2f} | Fee: ${trade[7]:.2f}")
            ...
            print(f"  Winning trades: {len(winning_trades)} ({len(winning_trades)/len(recent_trades)*100:.1f}%)")
            print(f"  Losing trades: {len(losing_trades)} ({len(losing_trades)/len(recent_trades)*100:.1f}%)")
