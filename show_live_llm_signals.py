...
def simulate_live_trading_with_llm():
    ...
    for i, (signal, confidence) in enumerate(zip(signals, confidences), 1):
        ...
        if signal in ['BUY', 'SELL']:
            ...
            enhancement = llm_advisor.enhance_trading_signal(signal, confidence, current_market_data)
            ...
            print(f"   📈 Confidence: {original_confidence:.1%} → {enhanced_confidence:.1%}")
            ...
            print(f"   {color} Final Decision: {decision} (AI-Enhanced: {enhanced_confidence:.1%})")
        else:
