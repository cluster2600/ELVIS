        ...
        if live_positions:
            ...
            for pos in live_positions[:5]:
                ...
                if len(pos) >= 5:
                    ...
                    try:
                        response = requests.get(f"https://fapi.binance.com/fapi/v1/ticker/price?symbol={symbol}", timeout=5)
                    ...
                    position_text = f"{symbol} {side} {qty_display} @ ${entry_price:.0f}"
                    ...
                    print(f"   ❌ Position skipped: insufficient fields ({len(pos)} < 5)")
