...
class StreamProcessor:
    def __init__(self, config: Dict):
        ...
        self.quality_metrics = {"latency": [], "throughput": [], "error_rate": 0}
    ...
    def _clean_old_metrics(self):
        ...
        self.quality_metrics["latency"] = [
            lat for lat in self.quality_metrics["latency"] if lat < retention_period
        ]

        self.quality_metrics["throughput"] = [
            ts for ts in self.quality_metrics["throughput"] if ts > cutoff_time
        ]
    ...
    def get_quality_metrics(self) -> Dict:
        ...
        return {
            "latency_avg": (
                np.mean(self.quality_metrics["latency"])
    ...
    def get_buffer_stats(self) -> Dict:
        ...
        return {
            ...
            "oldest_timestamp": (
                min(item["timestamp"] for item in self.data_buffer)
                if self.data_buffer
                else None
            ),
            "newest_timestamp": (
                max(item["timestamp"] for item in self.data_buffer)
                if self.data_buffer
                else None
            ),
        }
