...
class EnsembleModel:
    ...
    def _initialize_base_models(self):
        ...
        for model_config in model_configs:
            ...
            self.base_models.append(
                {"model": model, "type": model_type, "config": model_config}
            )
    ...
    def _train_neural_network(
        self, model: nn.Module, features: np.ndarray, targets: np.ndarray
    ):
    ...
    def predict(self, data: pd.DataFrame) -> Dict:
        ...
        with torch.no_grad():
            ...
            ensemble_prediction = (
                self.meta_learner(meta_features_tensor).cpu().numpy().flatten()
            )
        ...
        return {
            ...
            "base_predictions": dict(
                zip([m["type"] for m in self.base_models], base_predictions)
            ),
            "base_confidences": dict(
                zip([m["type"] for m in self.base_models], base_confidences)
            ),
        }
    ...
    def save_model(self, path: str):
        ...
        model_state = {
            "base_models": [
                {
                    ...
                    "state": (
                        m["model"].state_dict()
                        if isinstance(m["model"], nn.Module)
                        else m["model"]
                    ),
                }
            ...
            "meta_learner": (
                self.meta_learner.state_dict() if self.meta_learner else None
            ),
            "scaler": self.scaler,
        }
    ...
    def load_model(self, path: str):
        ...
        for model_info in model_state["base_models"]:
            ...
            self.base_models.append(
                {"model": model, "type": model_type, "config": config}
            )
