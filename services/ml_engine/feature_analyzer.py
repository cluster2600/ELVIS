...
class FeatureAnalyzer:
    ...
    def analyze_permutation_importance(
        self, model: Union[nn.Module, object], data: pd.DataFrame, targets: np.ndarray
    ) -> Dict:
    ...
    def analyze_shap_values(
        self, model: Union[nn.Module, object], data: pd.DataFrame
    ) -> Dict:
    ...
    def plot_feature_importance(
        ...
        plt.barh(
            ...
            xerr=importance_std[sorted_idx] if importance_std is not None else None,
        )
    ...
    def plot_shap_summary(self, shap_values: np.ndarray, features: np.ndarray) -> None:
        """Plot SHAP summary plot."""
        shap.summary_plot(
            shap_values, features, feature_names=self.feature_names, show=False
        )
        plt.tight_layout()
        plt.show()
    ...
    def generate_report(
        self, data: pd.DataFrame, model: Union[nn.Module, object], targets: np.ndarray
    ) -> Dict:
        ...
        self.plot_shap_summary(
            shap_analysis["shap_values"], self.prepare_features(data)[0]
        )

        self.plot_correlation_heatmap(correlations["correlation_matrix"])
