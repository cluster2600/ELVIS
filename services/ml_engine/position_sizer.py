...
class PositionSizingAgent:
    def __init__(self, config: Dict):
        ...
        self.policy_net = PositionSizingNetwork(self.state_dim, self.action_dim).to(
            self.device
        )
        self.target_net = PositionSizingNetwork(self.state_dim, self.action_dim).to(
            self.device
        )
    ...
    def prepare_state(
        self, market_data: pd.DataFrame, portfolio_state: Dict
    ) -> np.ndarray:
        ...
        market_features.append(
            market_data["close"].pct_change().rolling(20).std().values[-32:]
        )
    ...
    def calculate_reward(
        self, portfolio_state: Dict, new_portfolio_state: Dict
    ) -> float:
        """Calculate reward based on portfolio performance."""
        # PnL component
        pnl_reward = (
            new_portfolio_state["unrealized_pnl"] - portfolio_state["unrealized_pnl"]
        )
        ...
        # Drawdown penalty
        drawdown_penalty = -0.2 * max(
            0, portfolio_state["max_drawdown"] - new_portfolio_state["max_drawdown"]
        )
    ...
    def train_step(self, market_data: pd.DataFrame, portfolio_state: Dict) -> float:
        ...
        new_portfolio_state = self._execute_position_size(
    ...
    def _execute_position_size(
        self, position_size: float, portfolio_state: Dict
    ) -> Dict:
        ...
        new_portfolio_state = {
            ...
            - abs(new_position),
            "max_drawdown": max(
                portfolio_state["max_drawdown"], portfolio_state["unrealized_pnl"]
            ),
        }
